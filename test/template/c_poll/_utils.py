...
class _CPollRuntime(NativeRoleSupport):
    def __init__(
        ...
        self._set_event_checks = self._bind_function(
            '{prefix}_set_event_checks',
            argtypes=[ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p],
        )
        ...
        self._event_context_struct = _EventContextStruct
        self._hook_fn = ctypes.CFUNCTYPE(None, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p)
        self._event_check_fn = ctypes.CFUNCTYPE(ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p)
        ...
        event_fields = [
            (self._event_field_names[path], self._event_check_fn)
            for path in self._event_paths
        ] or [('_unused_placeholder', ctypes.c_int)]
        ...
        class _EventChecksStruct(ctypes.Structure):
            _fields_ = event_fields

        self._hooks_struct = _HooksStruct
        self._event_checks_struct = _EventChecksStruct
        self._hook_callbacks = []
        self._hook_values = None
        self._event_check_callbacks = []
        self._event_check_values = None
        self._current_cycle_event_ids = set()

        if auto_install_event_checks and self._event_paths:
            self.install_event_checks()
    ...
    def install_event_checks(self, callback_map=None):
        ...
        for event_path in self._event_paths:
            field_name = self._event_field_names[event_path]
            py_callback = callback_map.get(field_name)

            if py_callback is None:
                def _make_default_callback(expected_path):
                    @self._event_check_fn
                    def _callback(machine_ptr, ctx_ptr, user_data):
                        (void_machine_ptr, void_user_data) = (machine_ptr, user_data)
                        del void_machine_ptr, void_user_data
                        return 1 if self._event_ids[expected_path] in self._current_cycle_event_ids else 0

                    return _callback

                callback = _make_default_callback(event_path)
            else:
                def _make_callback(fn):
                    @self._event_check_fn
                    def _callback(machine_ptr, ctx_ptr, user_data):
                        (void_machine_ptr, void_user_data) = (machine_ptr, user_data)
                        del void_machine_ptr, void_user_data
                        ctx = ctypes.cast(ctx_ptr, ctypes.POINTER(self._event_context_struct)).contents
                        return 1 if fn(_EventContextView(self, ctx)) else 0
        ...
        self._event_check_callbacks = callbacks
        self._event_check_values = self._event_checks_struct(**kwargs)
        self._set_event_checks(self._machine, ctypes.byref(self._event_check_values), None)
...
@contextmanager
def render_c_runtime(dsl_code, auto_install_event_checks=True):
...
def build_c_runtime(dsl_code, initial_state=None, initial_vars=None, auto_install_event_checks=True, parameters=None):
    ast_node = parse_with_grammar_entry(
        textwrap.dedent(dsl_code).strip(),
        entry_name='state_machine_dsl',
    )

...
from ..solver.budget import SolveBudget
...
class _SolveBudget(SolveBudget):
    """BMC compatibility adapter for the shared symbolic-check deadline.

    Invalid budgets retain the existing :class:`BmcBuildError` contract.

    :param timeout_ms: Positive milliseconds, or ``None`` for no deadline.
    :type timeout_ms: Optional[int]
    :raises BmcBuildError: For an invalid timeout.
    """
    ...
    def __init__(self, timeout_ms: Optional[int]) -> None:
        try:
            ...
            # SolveBudget rejects invalid timeout values with ValueError;
            # the existing BMC entry points promise BmcBuildError instead.
            raise BmcBuildError(str(error)) from error

...
3. 它把事件触发和抽象动作实现留给用户手写函数  
   Python 版本更适合改成**继承生成类并覆写 protected hook**，而不是要求用户改生成文件。
...
- abstract action 通过子类覆写受保护 hook 扩展
...
- [x] 明确模板扩展路径只保留 protected hook override，不在模板运行时中保留 handler 注册系统
- [x] 在 `config.yaml` 中更多使用模板辅助函数，减少 `j2` 文件内重复展开的样板逻辑
- [x] 将在模板文件中高频重复出现的片段抽到统一 helper / macro / config 生成入口中，保证输出一致性
- [x] 将 `python` 模板的自定义表达式 style 尽量收敛为基于 `base_lang: python` 的最小覆盖，避免在 `config.yaml` 中重复抄写整套表达式模板
...
- 把模板系统从“实现已经存在”推进到“用户能真正理解并自己编写模板”
- 在 `docs/source/tutorials/` 中建立面向模板作者的成体系教程，讲清楚模板机制、文件结构、Jinja2 用法、测试方法与发布链路
- 明确现有 `docs/source/tutorials/render/` 教程内容已经陈旧，需要按当前模板系统实现与 builtin template 发布链路重写
- 将 builtin `python` 模板降为教程末尾的一个实例，而不是模板教程本身的主体

Checklist：

- [ ] 重构 `docs/source/tutorials/` 中与模板系统相关的章节结构，单独整理“模板系统”主线教程，而不是把模板内容零散夹在 builtin template 或其他功能教程中
- [ ] 明确重写现有 `docs/source/tutorials/render/index.rst`：
  现有内容过于陈旧，未能正确反映当前模板系统、builtin template 发布链路与测试方式，不能只做小修小补
- [ ] 为重写后的 render / template 教程同步提供中文版本：
  至少补齐 `docs/source/tutorials/render/index_zh.rst`，并保证中英两边的主题、结构与核心结论一致
- [ ] 为模板系统教程提供完整的英文与中文路径，并保持章节主题与顺序一致
- [ ] 在教程开头先解释模板系统解决什么问题：
  输入是什么、输出是什么、renderer 做什么、不做什么
- [ ] 明确讲清模板目录结构与文件职责：
  `config.yaml`、`.j2` 模板文件、静态文件、ignore 规则、模板目录 README、自带模板打包产物之间的关系
- [ ] 单独介绍 Jinja2 在本项目中的最小必要知识：
  变量、控制流、macro、filter、global、test，以及本项目为什么选择 Jinja2
- [ ] 讲清 `config.yaml` 中各段配置的用途与边界：
  `expr_styles`、`stmt_styles`、`globals`、`filters`、`tests`、`ignores`
- [ ] 讲清模板渲染时可直接使用的核心输入对象与常见访问方式：
  `model`、`model.walk_states()`、状态/事件/动作对象、变量定义等
- [ ] 讲清模板系统当前的生成机制与规模边界：
  哪些文件会被渲染、哪些文件会被原样拷贝、固定输出文件名的约束、目录级输出组织方式
- [ ] 在教程中说明何时应把逻辑放进 `config.yaml` helper，何时应写在 `.j2` macro，何时应直接展开到模板正文
- [ ] 在教程中说明模板作者应如何保持生成代码可读、稳定、易测试，包括命名、重复片段抽取、格式控制等约定
- [ ] 单独提供“如何从零写一个最小模板”的教程：
  从最小 `config.yaml` + 一个 `.j2` 文件开始，完成一次可运行的输出
- [ ] 单独提供“如何测试模板”的教程：
  覆盖 renderer 级测试、生成结果测试、行为对齐测试、必要时的 CLI 端到端测试
- [ ] 在教程中明确区分三类文档职责：
  模板系统 tutorials 面向模板作者；模板目录 README 面向模板维护者；生成 README 面向最终生成产物使用者
- [ ] 在教程末尾再补 builtin `python` 模板作为实例：
  用它说明前面讲过的机制如何落地，而不是把它当作模板系统定义本身
- [ ] 在 builtin `python` 模板实例章节中，补充“如何生成、如何 import、如何 `cycle()`、如何 override protected hook、如何 hot start”的最小示例
- [ ] 在模板系统教程中删除或改写任何把模板运行时描述为 handler 注册系统的内容，避免与当前结论冲突
- [ ] 为 `test_cli` 增加 builtin template 端到端测试：
  通过 CLI 使用自带 `python` 模板生成代码，再对生成结果执行最小功能验证
- [ ] 端到端 CLI 测试至少覆盖：
  `--template python`、输出目录生成成功、可 import 生成的 `machine.py`、可执行一次简单状态推进
- [ ] 收口生成 README 中的示例代码排版，去掉 `initial_vars` 示例里不必要的空行，例如：
  `machine = TrafficLightMachine(initial_state='TrafficLight', initial_vars={'a': 0, 'b': 0, 'round_count': 0})` 这一类示例应保持紧凑、美观、可直接复制
- [ ] 复查模板 README、生成 README、tutorials 三处示例代码风格，避免同一概念在不同文档里出现不一致写法

完成标准：

- docs/tutorials 中存在清晰、可顺着走通的模板系统教程路径，且英文/中文两边都能覆盖核心流程
- 用户读完教程后，能够理解模板系统机制，并能从零写出、调试并测试一个最小模板
- builtin `python` 模板在教程中只是落地实例，而不是模板系统概念本身
- `test_cli` 能证明“用户通过 CLI 调 builtin template 生成代码并跑通最小状态机”这条链路没有断
- 生成 README 中的热启动与扩展示例排版整洁，不再出现无意义空行或影响复制体验的格式问题

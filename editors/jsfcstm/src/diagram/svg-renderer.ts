...
function drawEdge(edge: FcstmElkEdge, ox: number, oy: number, out: string[]): void {
    ...
    // Transition labels: by default rendered with a white stroke halo so
    // they stay legible across edges. Note-effect labels get a folded
    // sticky-note background in PlantUML style instead.

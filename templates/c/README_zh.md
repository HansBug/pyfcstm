...
- 目标语言为 C99 以及 C++98 或更高版本，仅依赖常见标准库设施
...
- 不生成额外的 handler 注册体系
- 会对生成标识符做 C / C++ 双兼容规整，规避 C++ 关键字冲突
- 对公开整数 ABI 使用生成出来的 64 位整数别名，而不是直接暴露裸 `long long`
...
- 当前热路径已经切换到状态专用分发实现，validation / rollback 路径也已完成减拷贝优化，
  现有 C 模板回归测试已通过。
- 当前模板目标也会用 C++ 编译器做检查，并包含严格的 `c++98`
  代表性生成产物编译路径。
- 当前回归测试里也包含若干 C++ harness，用于覆盖基础行为、hot start、
  回滚行为、关键字安全标识符以及 abstract hook 挂载。

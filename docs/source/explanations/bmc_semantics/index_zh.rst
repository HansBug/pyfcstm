...
先看一条很短的门锁轨迹。假设门控制器从 ``Door.Locked`` 和 ``latch_engaged=1`` 开始。
一次维护事件打开门，却没有释放门闩。边界为 2 时，编码范围包含帧
:math:`F_0,F_1,F_2` 与步 :math:`0,1`：:math:`F_0` 是第一个被编码周期前的快照，第 0 步
是从 :math:`F_0` 到 :math:`F_1` 的完整运行时周期，第 1 步是从 :math:`F_1` 到 :math:`F_2` 的
完整运行时周期。在本页中，**步（step）就是一个 FCSTM 宏步（macro-step）**\ ：它是相邻
可观测帧之间的一条符号边，包含该周期内被选中的转换，以及按顺序执行的
入口、退出、效果或 ``during`` 动作。

全文使用下列集合：

* :math:`N` 是查询边界；帧索引为 :math:`0..N`，步索引为 :math:`0..N-1`。例如，门锁
  边界 :math:`N=2` 有三个帧和两个宏步。
* :math:`V` 是持久变量集合，:math:`\tau(v)` 是变量 :math:`v` 的 Z3 类型。例如，
  ``latch_engaged`` 是 :math:`V` 的一个成员。
* :math:`\mathcal{A}` 是全限定事件路径集合。例如，``Door.MaintenanceOpen`` 为每个步
  贡献一个事件输入。
* :math:`K_i` 是步 :math:`i` 的宏分支集合。例如，一个步可以选择维护转换分支或回退分支，
  但不能同时选择二者。
* :math:`\mathbb{B}` 是布尔域；:math:`S_0` 与 :math:`S_{\mathrm{rec}}` 是第 0 帧和递推帧的
  合法状态标识集合。

门锁轨迹先固定词汇。下一张表使用带有小整数值的已检入真实轨迹，方便审阅者不读长代码也能
核对公式。第 1 节随后把每个真实观测替换成 Z3 实际求解的符号族。
...
掩码和已接受分支依赖组装布尔模板；关系下沉再合取源状态等式。下式只命名条件本身；
其运行时定义性（runtime definedness）由 :eq:`bmc-case-relation` 携带，避免 Z3 的
总定义除法掩盖运行时除零。
...
被选中的分支必须可执行，不能只是布尔上可选择。令 :math:`\mathcal{D}^{Q}_{i,k}` 表示判定
宏条件 :math:`Q_{i,k}` 所需的路径敏感定义性，包括通过优先级或已接受分支依赖触达的
守卫原子。令 :math:`\mathcal{D}^{R}_{i,k}` 表示符号执行被选中分支的有序动作块时累积的
定义性。二者并集就是被选中分支的定义性载荷；精确并集与合取和受守卫关系一起定义在
:eq:`bmc-case-relation` 中。

例如，如果门锁维护分支的守卫是 ``latch_engaged / y > 0`` 且 :math:`y=0`，则
:math:`\mathcal{D}^{Q}_{i,k}` 包含 :math:`y\ne0`。如果它的效果写入
``latch_engaged = 1 / 0``，则 :math:`\mathcal{D}^{R}_{i,k}` 包含 :math:`0\ne0`。
任一合取项都会让被选中分支 UNSAT；未选中的分支不会把该合取项施加到当前步。
...
``_lower_bool_template`` 为条件和已接受分支守卫定义性贡献 :math:`\mathcal{D}^{Q}_{i,k}`，
``_prepare_case_lowering`` 为有序动作执行贡献 :math:`\mathcal{D}^{R}_{i,k}`，
``_build_case_relation`` 再把得到的 :math:`\operatorname{DefCase}_{i,k}` 约束加入
``consequent``。例如，被选中转换中的效果 ``x = 1 / 0`` 会贡献 :math:`0\ne0`；因为
:math:`0\ne0` 为假，当 :math:`A_{i,k}` 为真时，
:math:`A_{i,k}\Rightarrow(R_{i,k}\land\operatorname{DefCase}_{i,k})` 无法满足。该被选中
分支以及这次尝试执行均为 UNSAT。同一非法运算若位于未选中分支，则不会约束当前步，
因为定义性合取仍处于蕴含式后件中。反例：换成 :math:`A_{i,k}\land R_{i,k}` 会要求
所有展开分支同时被选中，通常令整个步 UNSAT；省略 :math:`\mathcal{D}_{i,k}` 同样
不可靠，因为 Z3 可以给部分算术运算任意赋值，从而伪造运行时无法执行的转换。
...
``_build_step_relation`` 与普通分支一样下沉回退，也包含
:math:`\operatorname{DefCase}_{i,k}`；回退运行时测试观察到被否定的已接受原子与
``during`` 更新。工作第 1、2 步选择 ``Root.B`` 回退并递增 ``counter``。短例子：如果较早
转换守卫需要 ``x / y`` 且 :math:`y=0`，回退不能把该未定义守卫当作“转换只是失败”的证据；
被选中的回退继承守卫定义性，该步为 UNSAT。反例：回退不必让变量全部停顿；这里它改变
``counter``，只有 ``ratio``、``keep`` 传递原值。
...
下表把本页的冻结关系公式逐项映射到 ``pyfcstm/bmc/relation.py``、语义回归测试与页首独立文档资源。

...
大多数 key 是可选的，默认等价于空配置。空的 `config.yaml` 会被视为一个空 mapping。未知 top-level key 会直接报错，而不是被静默忽略，这样配置拼写错误会在对应模板目录附近暴露。

`globals`、`filters` 和 `tests` 下的声明式条目支持三种形式：

| 条目形式 | 必填字段 | 含义 | Trust boundary |
| --- | --- | --- | --- |
| `type: template` | `template`，可选有序 `params` | 从内联 Jinja2 snippet 构造 callable。 | Trusted template source；仅 generation-time。 |
| `type: import` | `from` | import Python 对象并暴露给模板。 | Trusted template code boundary；不要用于不可信模板。 |
| `type: value` | `value` | 暴露 YAML literal value。 | 仅 generation-time。 |

Jinja2、PyYAML、`pathspec`、renderer helpers、imported filters 这类 generation-time dependencies 是允许的，因为它们只在 `pyfcstm` 生成阶段运行。除非 generated target language 明确拥有并通过 runtime policy 批准，否则这些依赖不能泄露为 generated runtime dependency。目标语言专属 helper 应由对应模板通过 `type: import` 加载；例如 C-family runtime helper 属于 `templates/c/config.yaml` 和 `templates/c_poll/config.yaml`，不属于默认 renderer environment。

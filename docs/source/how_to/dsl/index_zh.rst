...
从一个 root composite、一个 initial transition 和 child leaf states 开始。教程使用紧凑的 thermostat 示例作为 first runnable model：
...
Assignment 使用 runtime numeric expression。Top-level 和 import-preamble initializer 使用更窄的 ``init_expression`` 子集：literal、math constant、arithmetic、bitwise operator 和 unary math function，但不能读取 runtime variable，也不能使用 C-style ternary。Guard 使用 condition expression。Comparison 把 numeric expression 桥接为 condition。完整 precedence 和 operator 请看 expression reference。
...
当一个声明需要展开到多个 source states 时使用 forced transition。它可以指向 state 或 exit marker，也可以使用一个 local、chain、root 或 guard trigger。它不支持 combo ``+`` trigger，也不支持 ``effect`` block。
...
transition 需要多个 trigger terms 时使用 combo trigger。支持 ordinary combo 和 entry combo。Event term 与 guard term 之间使用 ``+``。Forced transition 是独立 shorthand，不接受 combo ``+`` trigger。
...
``: [condition]`` 是 combo guard alias；没有 combo term 时优先使用 ordinary single-guard spelling ``: if [condition]``。Combo expansion 使用 pseudo relay states。Duplicate event terms、guard aliases、constant guards、pseudo-name extension 和 reserved ``__combo`` names 会通过 diagnostics 报告。详细 expansion 语义见 :ref:`dsl-combo-relay-semantics-zh`。
...
.. list-table:: Diagnostic repair shortcuts
   ...
   * - ``W_GUARD_VARS_NEVER_CHANGE``
     - guard 是否只读取常量或从未被 ``during`` / ``effect`` 更新的变量。
   * - ``W_DURING_CONST_ASSIGN``
     - 常量初始化是否应该放进 ``enter``，而不是每个 active cycle 都写。
   * - ``W_COMBO_*``
     - 重新检查 combo trigger terms；forced transitions 不支持 combo ``+`` chains。

完整 code 说明见 :doc:`../../reference/diagnostics_codes/index_zh`。

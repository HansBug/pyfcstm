...
function findOperatorAtPosition(text: string, column: number, operator: string): number {
    let index = 0;
...
export function findHoverInfo(text: string, column: number, word: string): FcstmHoverDoc | null {
    ...
    for (const operator of operators) {
        ...
        if (index !== -1) {
            return HOVER_DOCS[operator] || null;
        }
    }
...
export async function resolveHover(
...
): Promise<FcstmHoverResult | null> {
    const line = document.lineAt(position.line).text;
    const word = getWordAtPosition(line, position.character);
    const hoverDoc = findHoverInfo(line, position.character, word);
    if (hoverDoc) {
        return {
            kind: 'doc',
            doc: hoverDoc,
    ...
    const importTarget = await getImportWorkspaceIndex().getResolvedImportAtPosition(document, position);
    if (!importTarget) {
        return null;
    }
    ...
    return {
        ...
        sourcePath: importTarget.entry.sourcePath,
        entryFile: importTarget.target.entryFile,
        resolvedFile: importTarget.target.resolvedFile,
        rootStateName: importTarget.target.rootStateName,
        explicitVariables: importTarget.target.explicitVariables,
        absoluteEvents: importTarget.target.absoluteEvents,
        range: importTarget.entry.pathRange,
    };
}

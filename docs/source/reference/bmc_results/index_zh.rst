...
与本页同目录的 ``bmc_cli_v1.schema.json``、:mod:`pyfcstm.bmc.witness` 和入口行为测试。
...
这个模式是参考资料，不是运行时依赖。它与本页放在同一目录，并且有意不放入
``pyfcstm`` wheel、源码发行包或独立可执行文件。需要结构校验的消费方应下载或随
自己的集成固定保存对应版本的模式，再读取本地副本：

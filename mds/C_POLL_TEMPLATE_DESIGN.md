...
`c_poll` 在生成物组织上的设计思想，应与现有 `c` 模板保持一致，并进一步明确如下：

- 生成的 `machine.c` **完全不需要考虑人类友好性**
- `machine.c` 应被视为**黑盒运行时实现**
- `machine.c` 的首要目标是**运行性能**
- 只要行为正确、ABI 稳定、测试通过，就不需要为了“看起来好懂”而牺牲性能或引入多余公开结构

这意味着：

- `.c` 内部可以保留专门化分发、展开后的状态推进代码、偏性能导向的内部辅助函数
- `.c` 不需要额外暴露给用户用于“阅读内部原理”的中间层
- `.c` 不需要为了人工浏览而刻意维持解释器式、表驱动式、教学式结构

与之对应，生成的 `machine.h` 才是面向用户的稳定接口层，应承担以下职责：

- 暴露确实需要给用户使用的公开 API
- 暴露确实需要给用户访问的数据结构与 typedef
- 暴露必要的 state id / event id / hook / context / vars 等公开契约
- 其余不属于用户接入面的内部结构、内部函数、内部状态，不应放进 `.h`

换句话说：

- `.c` 是黑盒高性能 runtime
- `.h` 是小而稳、以用户使用为中心的 public contract

除了把事件输入模型切换为 poll/hook 方式以外，`c_poll` 其余总体设计思想应尽量与现有 `c` 模板保持一致。

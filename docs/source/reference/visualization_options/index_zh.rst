...
它们的人来说都足以识别是哪幅图。再次展示同一幅图会返回同一个文件，你的另一个
进程也是如此——除非那个目录被发现属于他人或对他人开放，此时每个进程各用自己的一份，
并给出一条警告。在 Windows 上这依赖 ``%TEMP%`` 按账户隔离（默认如此）：多用户共享的
``TEMP`` 在这里无法检测，且 CPython 3.12.4 之前目录权限在该平台不会被应用——这种场景
请自己指定输出路径。

...
解码（witness decode）、运行时重放（runtime replay）、退出状态、错误和打包后的模式（schema）
发现方式。这是有界结果协议：有界结论（verdict）成功不等于无界证明。

本页的事实源是 :mod:`pyfcstm.entry.bmc`、
``pyfcstm/entry/bmc_cli_v1.schema.json``、:mod:`pyfcstm.bmc.witness` 和入口测试
``test/entry/test_bmc.py``。JSON 类型和必需键以模式为准；执行顺序、标准输出/标准错误、
文件副作用和退出状态以入口模块为准。
...
.. list-table:: 选项
   ...
   * - ``--timeout-ms``
     - 整数，``>= 1``
     - 未设置；Z3 无超时
     - 独立应用于每次 Z3 ``check()``。它不是 CLI 总墙钟预算，也不限制加载、解析、
       展开、公式构造、见证解码或重放。
   * - ``--max-bound``
     - 整数，``>= 1``
     - 未设置；无 CLI 上限
...
#. 求解主性质目标。
#. 如果性质暴露非常假的边界不完整公式，再独立求解
   该诊断公式。
#. 如果主结果为 SAT，把模型解码为 ``bmc-witness/v1`` 轨迹，并以
   ``abstract_handlers=None`` 交给 ``SimulationRuntime`` 重放。
...
.. list-table:: 进程退出状态
   ...
   * - ``2``
     ...
     - 修复缺失/未知选项，数值必须为正整数。
   * - ``3``
     - 求解器 ``unknown``/``timeout`` 或 ``response`` 边界 ``incomplete``。
     - 完整报告；``witness`` 和 ``replay`` 为 ``null``。
...
人类可读输出先报告有界性质结论，再展示求解器机制。首行严格采用以下四种形状之一：
...
显示每次检查超时、响应窗口检查状态与耗时、求解器原因和诊断。SAT 结果再显示重放状态与
...
.. list-table:: 顶层 ``bmc-cli/v1`` 字段
   ...
   * - ``witness``
     - 对象或 ``null``
     - 是
     - 主目标 SAT 时为 ``bmc-witness/v1``，否则为 ``null``。
   * - ``replay``
     - 对象或 ``null``
     - 是
     - 主目标 SAT 时为运行时重放结果，否则为 ``null``。
   * - ``exit_code``
     - ``0, 1, 3, 4`` 之一
     - 是
...
``witness`` 仅在主目标 SAT 时存在，且
``schema_version == "bmc-witness/v1"``。根节点必需字段如下：
...
.. list-table:: 见证根节点与嵌套记录
   ...
   * - ``witness.solver``
     ...
     - ``status`` 固定为 SAT；SAT 的原因为 ``null``；次状态可为任一状态或 ``null``。
   * - ``witness.initial``
...
每种性质都执行一次主检查。只有公式具有非常假的边界不完整观察时才执行
第二次检查；当前非平凡情形是 ``response``。每次检查都独立获得完整 ``--timeout-ms``。因此两次
100 毫秒检查的总耗时可以超过 100 毫秒，CLI 总耗时还可能更长。
...
``bmc-witness/v1`` 均不提供稳定、机器可读的原因判别字段。人可以结合原查询和轨迹
...
``incomplete_status: sat``、``outcome: incomplete``、见证/重放为 ``null``，且
``exit_code: 3``。如需确定边界，应增大查询边界。
...
模式资源是 ``pyfcstm/entry/bmc_cli_v1.schema.json``。Setuptools 包数据
包含各包的 ``*.json``，``MANIFEST.in`` 也递归包含 ``pyfcstm`` 下的 JSON，
因此 wheel 和 sdist 都携带该文件。消费方应通过已安装包解析资源，不应使用仓库相对
路径。兼容 Python 3.7 的代码可使用 ``pkgutil.get_data``：
...
报告分支的代表性封装。打包冒烟测试至少应从源码树、已安装 wheel，以及任何
声称 CLI 一致性的独立发行版中读取资源。模式的 ``$id`` 是标识符；
已安装消费方不应要求联网获取它。
...
* 退出状态 ``3`` 是一个进程类别，但在调整超时或边界前，要区分 ``timeout``、``unknown`` 和
  ``response`` 不完整。

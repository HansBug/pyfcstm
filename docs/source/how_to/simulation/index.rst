...
.. list-table:: Simulator commands
   ...
   * - Command
     - Use it for
   * - ``cycle [count] [events...]``
     - Execute one or more cycles.  Event names after the count are passed to
       the cycle.
   * - ``current``
...
human-readable snapshots when YAML support is available in the installed
environment.  Keep exported files outside the source tree unless the file is a
checked-in demo output regenerated by the docs build.
...
2. Use ``pyfcstm simulate -i ../cli/simple_machine.fcstm -e "cycle; events; current" --no-color``

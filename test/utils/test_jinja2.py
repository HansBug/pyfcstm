...
class TestJinjaEnvironmentEnhancement:
    ...
    def test_to_c_public_identifier_avoids_reserved_public_shapes(self):
        cases = {
            ("Root", "Machine"): "RootMachine",
            ("_Root", "Machine"): "p_p5_z00005FRootMachine",
            ("class", "Machine"): "class_Machine",
            ("class", "_MACHINE"): "p_p5_class_MACHINE",
        }

        for (value, suffix), expected in cases.items():
            assert to_c_public_identifier(value, suffix) == expected
            assert "__" not in expected
            assert not expected.startswith("_")

        assert to_c_public_macro_identifier("Root", "_MACHINE") == "ROOT_MACHINE"
        assert (
            to_c_public_macro_identifier("_Root", "_MACHINE")
            == "P_P5_Z00005FROOT_MACHINE"
        )
        assert to_c_public_macro_identifier("class", "_MACHINE") == "P_P5_CLASS_MACHINE"
        assert is_c_public_identifier_reserved("_ROOT_MACHINE")
        assert is_c_public_identifier_reserved("ROOT__MACHINE")
        assert not is_c_public_identifier_reserved("ROOT_MACHINE")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "{% if 'hello' is string %}yes{% else %}no{% endif %}",
                {},
                "yes",
            ),  # Test 'is string' test
            (
                ...
                {},
                "[0, 1, 2]",
            ),  # Test range() and list() as filters
            ...
            (
                "{% if 42 is number %}yes{% else %}no{% endif %}",
                {},
                "yes",
            ),  # Test 'is number' test
            ...
            (
                ...
                {},
                "[3, 2, 1]",
            ),  # Test reversed() as filter
            (
                "{{ {'a': 1, 'b': 2}|items|list }}",
                {},
                "[('a', 1), ('b', 2)]",
            ),  # Test items() as filter
            (
                "{{ 'hello'|enumerate|list }}",
                {},
                "[(0, 'h'), (1, 'e'), (2, 'l'), (3, 'l'), (4, 'o')]",
            ),
            # Test enumerate() as filter
            (
                "{% if 'hello' is callable %}yes{% else %}no{% endif %}",
                {},
                "no",
            ),  # Test 'is callable' test
            (
                "{{ [1, 2, 3]|map('str')|list }}",
                {},
                "['1', '2', '3']",
            ...
            (
                ...
                {},
                "['a', 'b']",
            ),  # Test sorted() as filter
            (
                "{{ 'hello'|set|list|sort }}",
                {},
                "['e', 'h', 'l', 'o']",
            ),  # Test set() and sort() as filters
            (
                "{% if [1, 2] is iterable %}yes{% else %}no{% endif %}",
                {},
                "yes",
            ),  # Test 'is iterable' test
            ...
            (
                "{{ {'a': 1, 'b': 2}|dict|keys|list|sort }}",
                {},
                "['a', 'b']",
            ),  # Test dict(), keys(), and sort() as filters
            (
                "{{ 'hello'|list|set|list|sort }}",
                {},
                "['e', 'h', 'l', 'o']",
            ),  # Test list(), set(), and sort() as filters
            (
                "{% if 42 is instance(int) %}yes{% else %}no{% endif %}",
                {},
                "yes",
            ),  # Test 'is instance' test
            (
                "{% if int is subclass(object) %}yes{% else %}no{% endif %}",
                {},
                "yes",
            ),  # Test 'is subclass' test
            (
                "{{ 'hello'|getattr('upper')() }}",
                {},
                "HELLO",
            ),  # Test getattr() as filter
            (
                "{{ [1, 2, 3]|map('str')|join('-') }}",
                {},
                "1-2-3",
            ),  # Test map() and join() as filters
            (
                "{{ 'hello'|list|enumerate|list }}",
                {},
                "[(0, 'h'), (1, 'e'), (2, 'l'), (3, 'l'), (4, 'o')]",
            ),
            # Test enumerate() as filter
            (
                ...
                {},
                "[1, 4, 9]",
            ),  # Test map() with multiple arguments
            # ("{{ [1, 2, 3]|reduce('lambda x, y: x * y') }}", {}, "6"),  # Test reduce() as filter
            # ("{{ 'hello'|slice(None, None, -1) }}", {}, "olleh"),  # Test slice() with step
            (
                "{{ [1, 2, 3]|map('str')|map('len')|sum }}",
                {},
                "3",
            ),  # Test chaining multiple map() calls
        ],
    )
    def test_env_with_builtins(
        self, env_with_builtins, template_str, extra_params, expected_str
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "{{ 'Hello World'|to_identifier }}",
                {},
                "Hello_World",
            ),  # Test to_identifier filter
            (
                "{{ indent('Hello\\nWorld', '  ') }}",
                {},
                "  Hello\n  World",
            ),  # Test indent global function
            (
                "{{ PATH }}",
                {"PATH": "/usr/bin"},
                "/usr/bin",
            ),  # Test environment variable as global
            (
                "{{ 'Hello World'|normalize|upper }}",
                {},
                "HELLO_WORLD",
            ),  # Test normalize and upper filters
            (
                "{{ 'Hello123'|to_identifier }}",
                {},
                "Hello123",
            ),  # Test to_identifier with numbers
            (
                "{{ indent('Line1\\nLine2\\nLine3', '> ') }}",
                {},
                "> Line1\n> Line2\n> Line3",
            ...
            (
                "{{ 'Mixed Case'|normalize|to_identifier }}",
                {},
                "Mixed_Case",
            ),  # Test normalize and to_identifier together
            (
                "{{ indent('Single', '* ') }}",
                {},
                "* Single",
            ),  # Test indent with single line
        ],
    )
    def test_env_with_settings(
        self, env_with_settings, template_str, extra_params, expected_str
    ):

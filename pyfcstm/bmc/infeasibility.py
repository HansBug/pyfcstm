...
def _is_domain_group(group: BmcTrackedConstraint) -> bool:
    """Return whether a kernel group belongs to the domain aggregate."""
    return group.stage == "kernel" and group.category.startswith("domain")


def _is_transition_group(group: BmcTrackedConstraint) -> bool:
    """Return whether a kernel group belongs to the transition aggregate."""
...
AGGREGATE_SELECTORS: "Mapping[str, object]" = MappingProxyType(
    {
...
def partition_tracked_groups(core) -> TrackedGroupPartition:
...
def _run_probe(
    ...
    if budget.deadline is not None and remaining is None:
        return "timeout", ProbeRecord(name, "timeout", False, 0.0)
    ...
    return resolved, ProbeRecord(name, resolved, True, elapsed)


def classify_infeasibility(
    core, stage: str, budget: _SolveBudget
...
def extract_source_core(core, scope: str, budget: _SolveBudget) -> CoreExtraction:
    ...
    :param budget: Budget shared with the mandatory solve.
    :type budget: pyfcstm.bmc.solver._SolveBudget
    :return: Ordered core groups, or a degraded status with a reason.
    :rtype: CoreExtraction
    :raises pyfcstm.bmc.errors.BmcBuildError: If the scope is unknown, a label
        cannot be mapped back to exactly one in-scope group, or the extracted
        core no longer re-checks as unsatisfiable.
    ...
    for group in targets:
        ...
        if label_name in by_label:
            raise BmcBuildError(
                "duplicate activation label for stable id %r." % group.stable_id
            )
    ...
    if status == "sat":
        return CoreExtraction(
            (),
            "unknown",
            "target formula for scope %r is satisfiable under its labels" % scope,
            (record,),
        )
    ...
    for literal in solver.unsat_core():
        ...
        if name in seen:
            raise BmcBuildError("unsat core returned duplicate label %r." % name)
        seen.add(name)
    ...
    if verifier.check() != z3.unsat:
        raise BmcBuildError(
            "extracted source core for scope %r does not re-check as unsat." % scope
        )
    return CoreExtraction(ordered, "complete", None, (record,))

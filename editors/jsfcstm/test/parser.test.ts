...
describe('jsfcstm parser real grammar coverage', () => {
    const validCases = [
        ...
        'state Root { event testEvent named "Event with \\"escaped\\" quotes"; }',
        'state Root { event fileEvent named "File\\nNew\\tLine"; }',
        'state Root { import "./pair.fcstm" as Pair { def {a, b, c} -> pair_*; def a_*_b_* -> pair_${1}_${2}; def * -> pair_$0; event /Stop -> /System.Stop; } }',
        'state Root { import "./complex.fcstm" as Complex named "Complex Module" { ; def sensor_* -> io_$1; ; def x_*_y_*_z_* -> xyz_${1}_${2}_${3}; event /Start -> Start; ; event /Reset -> /Plant.Reset named "Plant Reset"; } }',
        'state Root { enter abstract setupFunc /* Initialize state variables */ exit onExit ref utils.init; }',
        'state Root { during before funcName { result = sin(angle) + cos(angle); } during after abstract funcName /* after aspect documentation */ }',
        'state Root { >> during before funcName ref chain.id; >> during after abstract AuditHook; }',
        'state Root { !* -> [*] : if [(x > 5) ? true : false]; }',
        ...
        'state Root { state A; state B; A -> B : if [(a ^ b > c ^ d) xor (e ^ f > g ^ h)]; }',
        ...
        'state NamedOrder named "Main" { state ChildA named "A"; event Ev1 named "One"; state ChildB named "B"; event Ev2 named "Two"; }',
        'pseudo state ControllerState { event StartEvent; event StopEvent named "Stop Operation"; event ResetEvent named "System Reset"; }',
    ];

...
| 默认共享 runner | simulation, generated_python_alignment=143 | 通过：当前共享 corpus 默认覆盖模拟器与生成 Python 对齐。 |
| exclude-only runner 选择 | runners 字段=0；exclude_runners 字段=0 | 通过：当前 corpus 只使用默认 runner 集合加排除例外。 |
| 契约外 top-level 字段 | 0 | 通过：未出现 boundary、runners、runtime_options、model_build、commands 或 expected_failure。 |
| 契约外观察字段 | 0 | 通过：未出现 stack、cycle_count、history*、return、logs、warnings 或错误诊断字段。 |
| README 索引 | clean | README fixture index matches YAML runners and assertion types. |
...
| warnings | 0 |
| logs | 0 |
| abstract_handler_errors | 0 |
| error_state | 0 |
| error_info | 0 |
| anonymous_warning_count | 0 |
| output_contains | 0 |
| output_not_contains | 0 |
| error_contains | 0 |
| should_exit | 0 |

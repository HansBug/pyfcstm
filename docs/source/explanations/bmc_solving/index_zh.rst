...
同一个具名动作调用两次时，源码相同，但引用分别属于 ``action 1`` 和 ``action 2``，
局部变量生命周期也独立。在两个调用之后求值的守卫读取第二次调用的输出。
...
并排除先前 Undo 迁移被接受。状态比较保留原生运算符和数字编码，旁边注明模型名称；
事件使用模型完整路径和步骤编号。参数跨步共享，外部输入逐步变化。
...
``expanded=True`` 才增加原生展开公式，不替换原有绑定关系。

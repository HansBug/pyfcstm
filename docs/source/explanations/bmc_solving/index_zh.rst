...
三种方法分担这件事。``input`` 节点用 ``core_binding``：归一化事实被从零重新编码，
...
``solver_entailment`` 预留给改由求解器判定的 ``derived`` 与根步骤。当前目录不使用
它；在此点明它的名字，是为了让未来某个携带它的节点读起来是可理解的而非突兀的。
...
今天没有查询能走到这个方法，所以这一对数字是已定义而未发布的。会分解成多条要求的组
只有步关系，而前提里含有它某个 case 的规则只有 ``transition_assignment``\ ——正是
可达性表里标为不可达的那条。它为什么不触发，见下一节。
...
另有两条规则出于同一原因不可达，只是隔了一步。``equality_substitution`` 需要一个
可供代入的派生事实，``arithmetic_evaluation`` 需要一个 ``arithmetic_expression``
事实——而后者只有 ``transition_assignment`` 会产出。从冲突核成员到证明事实的翻译只
发出七种 kind，其中并没有它，所以这两条链缺的是第一环而不是最后一环。这正是目录既
封闭又完整、而九条规则里仍有三条从不触发的原因：它们的前提没有任何冲突核成员能陈述。

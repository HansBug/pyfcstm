...
5. 如果 B 是复合状态，按“初始转换决策/效果 → 普通 `during before` → 子状态进入”的顺序处理初始转换
...
当进入复合状态 P 时：
1. 执行 P 的 enter 动作
2. 按定义顺序检查 P 的初始转换（init_transitions）
3. 找到第一个满足条件且可到达稳定边界的初始转换
4. 执行该初始转换的 effect（如果存在）
5. 执行 P 的普通 `during before` 动作
6. 进入被选中的子状态，重复上述过程

普通 `during before` 明确晚于本次初始转换的 guard 决策，因此它不能反向影响同一个复合状态本轮 `[*] -> Child` 的选择。
...
第1次 cycle:
  从 Root 开始 DFS 式尝试：
    Root.[*] -> System
    System.enter / System initial transition decision / System.during before 在模拟过程中可被尝试

...
术语说明：人工报告（human report）、结构化报告（structured report）和大语言模型报告（LLM report）在本页首次交接后，后文只使用中文术语。
...
大语言模型报告包含修复协议、源码摘录、``refs``、注册表摘要、建议动作和禁止做法。它故意比完整 JSON 更小。
...
* CI、大语言模型和验证支持的任务做法见 :doc:`../../how_to/inspect/index_zh`。

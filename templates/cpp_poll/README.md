# cpp_poll template maintainer guide

This guide is for changing the `cpp_poll` template. Downstream integration belongs in `README.md.j2` / `README_zh.md.j2`; the root [template handbook](../README.md) owns renderer, metadata and packaging contracts. Read the source map, preserve the behavior below, then select the checks for your change.
...
| Shared C core | Edit `../c_poll/machine.c.j2` / `machine.h.j2`; these files are symlinked here. Test both the core and wrapper suites |

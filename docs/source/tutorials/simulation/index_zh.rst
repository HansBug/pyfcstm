...
- **during before**\ （不带 ``>>``）：复合状态初始转换已选定后、进入被选中子状态前执行
...
- 组合 transition trigger（例如 ``E1 + [x > 0] + E2``\ ）会在运行前展开为伪状态链，因此遵循同一套验证和回滚规则
...
- 进入时，普通 ``during before`` 在初始转换守卫选择之后执行，因此不能影响同一次初始转换选择
- 伪状态跳过祖先切面动作
...
组合 trigger 展开为生成伪状态之后，也必须保持这个顺序。前缀共享只是实现细节，不能让后写的组合分支越过更早的普通转换或组合转换。

...
class BmcUnsatConstraint:
    ...
    Example::
        ...
        >>> BmcUnsatConstraint("guard", (z3.BoolVal(False),)).stable_id
        'guard'
    """
...
class BmcUnsatQuery:
    ...
    :type constraints: Tuple[BmcUnsatConstraint, ...]
    :param background: Named groups retained in every check, defaults to ``()``.
    :type background: Tuple[BmcUnsatConstraint, ...], optional
    ...
    Example::
        ...
        >>> explain_unsat_core(BmcUnsatQuery("empty", ())).solver_status
        'sat'
    """

    query_id: str
    constraints: Tuple[BmcUnsatConstraint, ...]
    background: Tuple[BmcUnsatConstraint, ...] = ()
    ...
    def __post_init__(self):
        ...
        if any(not isinstance(group, BmcUnsatConstraint) for group in groups):
            raise TypeError("query members must be BmcUnsatConstraint values")
...
class CoreExtraction:
    ...
    :type groups: Tuple[Union[BmcTrackedConstraint, BmcUnsatConstraint], ...]
    ...
    groups: Tuple[Union[BmcTrackedConstraint, BmcUnsatConstraint], ...] = ()
    status: str = "complete"
    reason: Optional[str] = None
...
def _trial_solver(groups, background=()) -> z3.Solver:
    ...
    :type groups: Sequence[Union[BmcTrackedConstraint, BmcUnsatConstraint]]
...
def _minimize_core(
    extraction: CoreExtraction, budget: _SolveBudget, background=()
) -> MinimizedCore:
    ...
    Example::
        ...
        >>> _minimize_core(empty, _SolveBudget(None)).reduction
        'raw'
    """
...
def explain_unsat_core(query, *, selected_ids=None, minimize=True, timeout_ms=None):
    ...
    Example::
        ...
        >>> query = BmcUnsatQuery("false", (BmcUnsatConstraint("rule", (z3.BoolVal(False),)),))
        >>> explain_unsat_core(query, minimize=False).core_ids
        ('rule',)
    """
    if not isinstance(query, BmcUnsatQuery):
        raise TypeError("query must be a BmcUnsatQuery")

...
class TestModelModel:
    ...
    def test_operation_basic(self, operation_1, operation_2):
        ...
        assert operation_1.expr == BinaryOp(
            x=Integer(value=0), op="+", y=Variable(name="b")
        )
        assert operation_1.to_ast_node() == dsl_nodes.OperationAssignment(
            name="b",
            expr=dsl_nodes.BinaryOp(
        ...
        assert operation_2.expr == BinaryOp(
            x=Integer(value=3),
            op="+",
            y=BinaryOp(
                x=Variable(name="a"),
        ...
        assert operation_2.to_ast_node() == dsl_nodes.OperationAssignment(
            name="b",
            expr=dsl_nodes.BinaryOp(
                expr1=dsl_nodes.Integer(raw="3"),
                op="+",
                expr2=dsl_nodes.BinaryOp(
    ...
    def test_var_defines(self, var_define_a, var_define_b):
        ...
        assert var_define_a.to_ast_node() == dsl_nodes.DefAssignment(
            name="a", type="int", expr=dsl_nodes.Integer(raw="0")
        )
        ...
        assert var_define_b.init == BinaryOp(
            x=Integer(value=2), op="|", y=Integer(value=5)
        )
        assert var_define_b.init() == 7
        assert var_define_b.to_ast_node() == dsl_nodes.DefAssignment(
            name="b",
            type="int",
            expr=dsl_nodes.BinaryOp(
                expr1=dsl_nodes.Integer(raw="2"),
    ...
    def test_state_LX_LX1(self, state_LX_LX1):
        ...
        assert state_LX_LX1.substate_name_to_id == {
            "LX11": 0,
            "LX12": 1,
            "LX13": 2,
            "LX14": 3,

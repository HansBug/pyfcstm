...
- 将默认 human inspect 报告改进为带相邻源码上下文的 checker-style 诊断布局，并新增 ``--color auto|always|never`` 控制 ANSI 颜色，同时保证文件、pipe 和机器格式不含 ANSI escape。
- 新增稳定的 LLM inspect 报告格式 ``--format llm-json`` 和 ``--format llm-md``，使用 ``pyfcstm.inspect.llm.v1`` schema，并提供源码上下文、provenance、repair guidance 和 do-not notes 供修复循环使用。

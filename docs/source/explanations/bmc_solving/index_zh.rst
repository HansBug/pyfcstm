...
令 :math:`C_N` 表示边界 :math:`N` 的核心转换关系，:math:`Q_N` 表示编译后的性质目标，
:math:`\Omega_N` 表示可选的“响应义务越过边界”观测。性质编译会产生两个可独立求解的公式：
...
第一次检查总会执行。只有当 ``check_incomplete`` 为真且 :math:`\Omega_N` 不是常假公式时，才执行第二次检查。
当前只有 ``response`` 性质拥有非平凡的尾部观测，因此第二次检查目前属于 ``response`` 的边界完整性机制。

:func:`pyfcstm.bmc.witness.solve_bmc_property` 对每个适用公式分别调用一次内部 ``_solve``。每次调用都会创建新的
``z3.Solver``，设置同一个 ``timeout_ms``，加入一个公式，再调用 ``check()``。因此超时值是\ **每次检查的预算**，
不是两次检查共享的总预算。如果两次检查都需要执行，超时值 :math:`T` 允许的求解器时间最坏约为 :math:`2T`，
另加构造和报告开销。即使主检查已经超时，也不会自动取消独立的尾部检查。

Z3 返回 ``unknown`` 后，实现会读取 ``reason_unknown()``：原因恰好为 ``"timeout"`` 时，公开状态是
``timeout``；其他原因保持为 ``unknown``。两种状态都不携带模型。主检查耗时写入 ``elapsed_ms``；尾部检查耗时以
``incomplete_elapsed_ms=...`` 诊断保留。禁用第二次检查也会留下 ``incomplete_check=disabled``，而不会被误写成
“已经证明不存在未完成后缀”。
...
尾部结果既不能削弱主公式 SAT 时已经找到的 ``response`` 反例，也不能影响其他性质类别。
公开的三值性质结论为：
...
``runtime.cycle()``。同样，分支标签、``delta``、``gamma`` 和 ``progress`` 只属于见证解释信息；运行时没有对应的
公开观测。
...
其中帧相等覆盖状态、终止标志、持久变量键和值；宏步相等覆盖输入事件、已消费与未消费事件，以及有序抽象调用的元数据和
快照。浮点值使用明确的回放容差，而不是逐位相等。初始哨兵与冷初始化实际产生的运行时状态比较，不会被误当成普通状态路径。
...
分支标签和求解器专用进度标志有意不属于 :math:`\operatorname{eq}_S`：运行时不可能对自己没有发布的信息产生分歧。
...
尾部查询会同时执行 :eq:`bmc-solve-formulas` 中的两个公式：
...
下表是本页求解与重放公式的前向审计图。字面 LaTeX 就是各公式目标处的带标签块；中英文文件使用完全相同的块。
...
.. list-table:: 冻结求解公式台账
   ...
   * - 公式与主张
     ...
     - 测试锚点
     - 可运行查询与轨迹
   * - :eq:`bmc-solve-formulas`：主检查与尾部检查分离

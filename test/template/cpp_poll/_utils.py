...
@contextmanager
def render_cpp_poll_artifacts(dsl_code):
...
def compile_and_run_cpp_poll_wrapper_harness(
    artifacts,
    stem,
    source_code,
    *,
    ...
    if cmake_executable is None:
        pytest.skip("cmake is required for generated C++ poll-wrapper tests.")
...
def available_compiler_pair(c_compiler_name, cxx_compiler_name):
    ...
    if c_compiler is None or cxx_compiler is None:
        pytest.skip(
            "{c_compiler_name}/{cxx_compiler_name} are required for this poll-wrapper compile gate.".format(
...
def compile_with_msvc_like_tool(artifacts, stem, source_code, tool_name):
    if os.name != "nt":
        pytest.skip(
            "{tool_name} poll-wrapper gate only runs on Windows.".format(
                tool_name=tool_name
            )
        )
    ...
    if compiler is None:
        pytest.skip(
            "{tool_name} is required for this poll-wrapper compile gate.".format(
                tool_name=tool_name
            )
        )

...
「可达」一列记录当前是否已知有查询能产出携带该规则的节点。\ **没有任何规则不可达**\ ：它们都是
消费方必须接受的封闭词表的一部分，但今天没有查询能走到它们，读者若为这类冲突选择
``proof`` 深度，得到的会是 ``formal``。原因是结构性的，详见
:doc:`/explanations/bmc_solving/index_zh`。

这三条也是结论不为矛盾本身的全部规则，因此只有它们能产出 ``derived`` 节点。今天发布
的证明里不含派生节点：读者看到的是每个子集极小成员一个 ``input`` 节点，加上唯一的
``contradiction`` 根。消费方仍须接受 ``derived``\ ，因为词表是封闭的，而这几行的取值
会变。
...
走到第六条的用例是同一份语料里的 ``event_conflict.fbmcq``\ 。实测比值请从那里读，而
不要从本页读：它是那份语料在某个版本上的性质，不是工具的性质。

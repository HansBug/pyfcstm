...
class TestSimulationWithSamples:
    ...
    def test_dlc1_traffic_light(self):
        ...
        # Initial cycle - enters InService but waits for Start event
        runtime.cycle()
        assert runtime.current_state.name == 'InService'
        assert runtime.vars['a'] == 0
        assert runtime.vars['b'] == 0
        assert runtime.vars['round_count'] == 0

        # Provide Start event - enters Red then immediately transitions to Green
        runtime.cycle(['TrafficLight.InService.Start'])
        assert runtime.current_state.name == 'Green'
        assert runtime.vars['b'] == 3  # Start effect: b=1, Red->Green effect: b=3
...
'''
        ...
        # During executes again before exit: 111 + 100 + 1000 = 1211
        assert runtime.vars['a'] == 1211
        assert runtime.is_ended

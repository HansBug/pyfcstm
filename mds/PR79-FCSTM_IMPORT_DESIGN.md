...
### 12.9 Phase 7: Model 顶层一键加载入口

本 phase 在 `model` 子模块下新增一个面向调用方的顶层快捷入口，把“DSL 文本 / 文件路径 -> AST 解析 -> import-aware model 构建”这一整段常见使用路径一键化，但保持现有底层能力和既有入口不变。该入口需要同时支持：

- 给定 `.fcstm` 文件路径直接加载为 `StateMachine`

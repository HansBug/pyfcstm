# FCSTM VSCode 扩展、editors/js 库与纯 JS Language Server 设计
...
| 0.3.0 | 2026-04-07 | 引入 `editors/js` 拆分方案，明确 npm 可发布 JS 库优先、VSCode 消费该库、拆分完成前不新增功能 | Codex |
...
第三，工程组织上不再建议把主要语义能力继续塞在 `editors/vscode/` 内部，而是明确建议先拆出一个位于 `editors/js/` 的、适合独立发布到 npm 的 FCSTM JS 库，再由 VSCode 扩展去 import 它。
...
- 语法解析与语义建模尚未分层
- 还没有一个可独立发布到 npm 的 `editors/js` 库承载主要 JS 能力
...
- 先构建位于 `editors/js/` 的、可独立发布到 npm 的 FCSTM JS 库
- 构建 **纯 JS runtime 的 FCSTM language server**
- 让 VSCode 扩展变成一个 **client + bundled server** 的标准结构，并消费 `editors/js`
- 所有核心 editor intelligence 能力都建立在 `editors/js` 提供的 Node.js 语义核心之上
...
- 先拆分
- 先维持现状
- 先把 `editors/js -> editors/vscode` 的依赖链走通
...
- `editors/vscode -> editors/js`
- `editors/js -/-> editors/vscode`

进一步说：

- language server handlers 可以位于 `editors/vscode/`，但其语义实现应调用 `editors/js`
- preview 可以位于 `editors/vscode/`，但其 diagram 数据和渲染核心应尽量来自 `editors/js`
- 除非明确属于 VSCode API 适配，否则新增逻辑默认先考虑放进 `editors/js`
...
- 先创建独立的 `editors/js/package.json`
...
- `core` 直接表达这是 JS 侧的核心语义库
- 在 VSCode、CLI、未来 web playground 等多个 JS consumer 中都自然
- 导入形式简洁：`import { ... } from '@pyfcstm/core'`
...
- `editors/js` 不依赖 `vscode`
- client 不重复实现语义逻辑
- server 通过标准 LSP 提供 editor intelligence，并优先调用 `editors/js`
- preview 不依赖 Python / Java / PlantUML / 网络服务
- 保持与当前 `engines.vscode = ^1.60.0`、`CommonJS`、`ES2015` 的兼容思路

---

## 6. `editors/js` 语义核心需要覆盖的对象

language server 不只是把现在的 provider 搬到另一个进程，而是要在 `editors/js` 里补齐一套真正可复用的语义模型。
...
- `editors/js` 与 `editors/vscode` 都继续采用 npm package 形式管理
...
除了 `editors/vscode` 需要升级为多入口 bundle，仓库里还需要新增 `editors/js` 的独立包构建。

推荐未来的构建关系是：

1. 先构建 `editors/js`
2. 再构建 `editors/vscode`
3. VSCode 扩展从本地包依赖中消费 `editors/js`
...
- `editors/js/package.json` 应具备清晰的 `name`、`main`、`exports`、`files`、`license`
- `editors/js` 应能单独执行 `npm pack`
- `editors/vscode/package.json` 在仓库内先通过本地依赖引用 `editors/js`
...
为了适配当前 repo 和 `engines.vscode` 约束，`editors/js` 和 `editors/vscode` 新引入依赖都建议满足：
...
- `editors/js` 需要能单独构建和打包
- `editors/vscode` 需要在消费本地 `editors/js` 的前提下稳定构建和打包
...
1. 最优先：创建并持有 `@pyfcstm` scope，然后发布 `@pyfcstm/core`
2. 过渡方案：如果你当前 npm 用户名就是 `hansbug`，先发布 `@hansbug/fcstm-core`
3. 最后兜底：临时使用 `fcstm-core`

如果你准备走正式方案 `@pyfcstm/core`，你需要额外完成：

* [ ] 在 npm 上创建或拿到 `pyfcstm` 这个 scope 的所有权
* [ ] 确认后续发布者也能加入这个 scope，而不是把发布权绑死在个人账号上

#### 8.6.2 `editors/js/package.json` 至少应包含什么
...
4. 先决定是发 `@pyfcstm/core` 还是过渡发 `@hansbug/fcstm-core`
5. 在 `editors/js/package.json` 里填好 `name`、`version`、`main`、`types`、`exports`、`files`、`publishConfig.access`
6. 执行 `npm run build`
7. 执行 `npm pack`
...
- 创建 `editors/js`
- 让 `editors/vscode` 稳定 import `editors/js`
- 把当前已有能力迁移到新边界下
- 在“不新增功能”的前提下维持现状
...
* [ ] 明确 `editors/js` 与 `editors/vscode` 的边界、职责、依赖方向和所有权
* [ ] 确认 `editors/js` 的目标定位是“可独立发布到 npm 的 FCSTM JS 库”
...
* [ ] `editors/js` 与 `editors/vscode` 的职责边界已经明确写清
* [ ] 当前扩展已有能力已经被列成可验收的 baseline
* [ ] feature freeze 规则已经明确：拆分完成前不启动新功能
* [ ] 后续阶段都以“先拆分、先维持现状”为默认前提

## Phase 1：建立 `editors/js` 可发布库骨架

### 目标

创建 `editors/js/`，先把它做成一个结构正确、可被 VSCode 本地消费、未来可发布到 npm 的独立 JS 库，但暂不在此阶段引入新功能。

### TODO

* [ ] 创建 `editors/js/` 目录及基础文件：`package.json`、`tsconfig.json`、`README.md`、`src/index.ts`
* [ ] 为 `editors/js` 设计清晰的包元数据：`name`、`version`、`main`、`exports`、`files`、`license`
* [ ] 为 `editors/js` 建立独立构建脚本和最小 smoke test
* [ ] 让 `editors/vscode/package.json` 通过本地依赖方式消费 `editors/js`
* [ ] 调整 `editors/vscode` 构建流程，使其在需要时先解析并使用本地 `editors/js`
* [ ] 保持 repo root 的 `make vscode` 路线可演进到“先 js、后 vscode”的顺序

### Checklist

* [ ] `cd editors/js && npm pack` 可以成功执行
* [ ] `cd editors/vscode && npm install` 可以成功解析本地 `editors/js` 依赖
* [ ] `editors/js` 已具备未来发布到 npm 所需的最小包结构
* [ ] 此阶段结束时，扩展功能没有新增，也没有因为拆出空壳包而回退

## Phase 2：把现有能力迁入 `editors/js` 并维持现状

### 目标

把现有 parser-backed 能力背后的主要 JS 逻辑迁到 `editors/js`，但此阶段仍然不新增功能，只要求维持现状并把依赖方向走通。

### TODO

* [ ] 将 parser adapter 从 `editors/vscode` 抽出到 `editors/js`
* [ ] 将可复用的 parse helpers、source range helpers、import resolution / workspace indexing 逻辑迁入 `editors/js`
* [ ] 将 VSCode 无关的数据提取逻辑迁入 `editors/js`，例如符号提取、补全候选构造、悬停元数据等
* [ ] 让 `editors/vscode` 中现有 providers 改为优先调用 `editors/js`
...
* [ ] `editors/vscode` 对 `editors/js` 的依赖链已经在真实构建中走通
* [ ] Phase 2 完成后，新增功能默认应先开发在 `editors/js`，而不是回写到 `editors/vscode`

## Phase 3：在 `editors/js` 中建立 AST、语义模型与 workspace graph

### 目标

在拆分完成且现状跑通之后，再开始真正的新内容开发，而且主要开发位置应在 `editors/js`。
...
* [ ] AST 和语义模型都位于 `editors/js`，而不是散落在 `editors/vscode`
...
在 `editors/js` 已具备 AST 和语义模型后，把当前智能能力迁移到标准 LSP server 中，形成正式的 `vscode client + bundled server + editors/js` 架构。
...
* [ ] server 的主要语义实现依赖 `editors/js`，而不是在 handler 里重写
...
在 `editors/js` 语义核心和 language server 都稳定后，再进入 references / rename / analyzers 等真正的新能力。

### TODO

* [ ] 在 `editors/js` 中补齐 symbol identity、references graph 和 rename planning 所需数据结构
...
* [ ] 新能力的主要实现仍然位于 `editors/js`，而不是回流到 `editors/vscode`

## Phase 6：纯 JS 可视化与预览

### 目标

在语义基础设施已经稳定之后，再基于 `editors/js` 提供的 diagram IR 和 renderer 能力构建预览。

### TODO

* [ ] 在 `editors/js` 中定义 diagram IR，使其由 semantic model 直接派生
* [ ] 在 `editors/js` 中设计纯 JS renderer，输出 SVG 作为首选预览格式
...
* [ ] diagram IR 与 renderer 主实现位于 `editors/js`
* [ ] SVG 导出结果稳定、可测试、可复现
...
把 `editors/js` 和 VSCode 扩展都推进到“可发布、可回归、可维护”的状态。

### TODO

* [ ] 扩展现有 `verify-*` 体系，新增 JS library、LSP、preview、package smoke test
* [ ] 为 activation time、server startup、diagnostics latency 建立性能基线
* [ ] 更新 `editors/js/README.md`、`editors/vscode/README.md`、教程文档、marketplace 文案、`TODO.md`
* [ ] 为 `editors/js` 准备 npm 发布前检查，例如 `npm pack` / `npm publish --dry-run`
* [ ] 为 VSIX 安装、升级、回滚、故障排查准备 release checklist
* [ ] 对老 VSCode 版本兼容性、bundle 体积、server 崩溃恢复做专项验证

### Checklist

* [ ] `editors/js` 已达到可发布 npm 包的基本质量门槛
...
如果 `editors/js` 没有真正成为主库，只是形式上创建目录，后续会出现三个问题：
...
本仓库对兼容范围非常敏感，因此 `editors/js` 和 VSCode 侧实现都不能轻易引入：
...
- 先创建位于 `editors/js` 的、适合独立发布到 npm 的 FCSTM JS 库
- 该库的正式长期包名建议定为 `@pyfcstm/core`
- VSCode 扩展应消费 `editors/js`，而不是继续把主要逻辑堆在 `editors/vscode`
...
**这件事的正确目标不是“继续在 `editors/vscode` 里边改边长功能”，而是“先拆出 `editors/js` 这个可发布 npm 库、让 VSCode 扩展消费它、在维持现状的前提下完成拆分，然后再把新的 language server 与可视化能力主要开发在这个 JS 库之上”。**

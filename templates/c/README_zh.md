...
本手册面向 `c` 模板维护者。下游接入说明由 `README.md.j2` / `README_zh.md.j2` 生成；根级[模板手册](../README_zh.md)负责渲染器、元数据和打包契约。维护时先查看源码分工，确认必须保持的行为，再选择对应检查。
...
| `machine.c.j2` | 高性能运行时实现模板 | `machine.c` |
| `README.md.j2` | 英文生成物用户手册 | `README.md` |
| `README_zh.md.j2` | 中文生成物用户手册 | `README_zh.md` |
| `config.yaml` | 渲染器配置、C 语句渲染、辅助函数命名和忽略规则 | 不复制 |
...
| 原生共有行为 | 同步 `c` 与 `c_poll`；改动也影响 `cpp` 和 `cpp_poll` 封装 |
...
生成时已知的有限元数据使用数值标识（numeric ID）和防碰撞宏，包括状态、事件、抽象动作、命名 `ref`、生命周期阶段、当前状态及活动叶状态。不要在 `ExecutionContext`、`EventContext` 或事件选择热路径重新引入字符串字段、`strcmp()`、逐周期字符串分配或格式化。字符串保留在 `last_error`、规范化 DSL、诊断路径、生成注释和 README 中；测试适配层可将数值标识单向映射为用例字符串，不能反向要求运行时携带字符串。

使用现有公开 C 标识符辅助函数保留路径边界、大小写及有效下划线。`Root.A.B` 与 `Root.A_B` 必须不同；不要通过统一大小写、合并或裁剪下划线生成规范名称。短兼容别名只有在完整命名域内唯一，且不碰撞计数宏、无效标记、阶段宏或规范标识时才允许生成，否则省略。所有公开名称都必须避开 C/C++ 保留形式（双下划线、下划线紧接大写字母）；同样适用于根状态机 ABI 前缀、可见性宏、回调初始化宏和头文件保护宏。`_Root`、`class`、`A__B` 等合法 DSL 根名也必须生成合法公开名称。

修改这些规则时，生成包含嵌套状态、相似路径、事件、抽象动作、命名引用和阶段的模型，检查公开头文件及执行热路径，再运行完整原生回归。`c` 和 `c_poll` 的共享约束保持一致，轮询事件上下文也使用数值标识；差异只能来自事件输入方式，并在两边手册中明确说明。

### 性能证据

历史基准使用中等规模的电梯模型，覆盖嵌套组合状态、初始与同级转换、退出到父状态及验证回滚。它以 `cc -O3 -DNDEBUG -std=c99` 比较当前专用化 C 运行时与早期字符串提交实现：

| 负载 | 周期数 | 平均加速比 | 中位加速比 |
| --- | ---: | ---: | ---: |
| `mixed` | 4,000,000 | 6.42x | 7.25x |
| `validation_heavy` | 8,000,000 | 9.59x | 9.75x |

这些历史结果支持数值事件标识、专用状态分派和减少验证复制的设计；它们不代表目标设备的速度，也不能替代语义回归。优化不得削弱回滚、动作行为和正确性。

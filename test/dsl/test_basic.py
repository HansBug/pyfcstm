...
class TestDSLBasicChainID:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "foo.bar",
                ChainID(path=["foo", "bar"]),
            ),  # Two identifiers connected with a dot
            ...
            (
                "this_is_snake_case",
                ChainID(path=["this_is_snake_case"]),
            ...
            (
                "x1.y2.z3",
                ChainID(path=["x1", "y2", "z3"]),
            ),  # Chain of identifiers containing numbers
            (
                "very_long_identifier.with.many.parts",
                ChainID(path=["very_long_identifier", "with", "many", "parts"]),
            ),
            # Long chain with multiple components
            (
                "a_1.b_2.c_3",
                ChainID(path=["a_1", "b_2", "c_3"]),
            ),  # Chain with underscores and numbers
            ("Class.static_method", ChainID(path=["Class", "static_method"])),
            # Common pattern for accessing static methods
            (
                "package.subpackage.Class",
                ChainID(path=["package", "subpackage", "Class"]),
            ),
            # Common import or qualification pattern
            (
                "x",
                ChainID(path=["x"]),
            ),  # Minimal valid chain_id with just one identifier
            (
                "a.b.c.d.e.f.g",
                ChainID(path=["a", "b", "c", "d", "e", "f", "g"]),
            ),  # Long chain with many segments
            ("_._._", ChainID(path=["_", "_", "_"])),  # Chain of underscore identifiers
            (
                "A1.B2.C3",
                ChainID(path=["A1", "B2", "C3"]),
            ),  # Chain with uppercase letters and numbers
        ],
    )
    ...
    def test_positive_cases_str(self, input_text, expected_str):
        assert (
            str(parse_with_grammar_entry(input_text, entry_name="chain_id"))
            == expected_str
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "foo..bar",
            ),  # Cannot have consecutive dots - each dot must be followed by an identifier
            ("1a",),  # Identifiers cannot start with a number
            (
                "foo.1bar",
            ),  # Each segment after a dot must be a valid identifier (cannot start with a number)
            ...
            (
                "foo.true",
            ),  # Reserved keyword 'true' can be used as identifier in this grammar
            (
                "sin.cos",
            ),  # Reserved function names can be used as identifiers in this grammar

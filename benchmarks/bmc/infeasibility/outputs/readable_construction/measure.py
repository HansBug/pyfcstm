...
for name, bound in [('dose_editor', 6), ('pool', 8)]:
    ...
    variants = {'compact': lambda: report.text_lines(),
    ...
    profiler.runcall(variants['compact'])
    solver_checks = sum(entry.callcount for entry in profiler.getstats()
                        if not isinstance(entry.code, str) and entry.code.co_name == 'check'
                        and entry.code.co_filename.endswith('/z3.py'))
    assert solver_checks == 0
    assert compiled.solve_formula.sexpr() == formula_before
    results[name] = {
        'bound': bound, 'cases': len(report.cases), 'selected_ids': selected_ids, 'compact_solver_checks': solver_checks,

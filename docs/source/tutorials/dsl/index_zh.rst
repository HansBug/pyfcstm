...
.. important::
   ...
   1. **进入**：进入复合状态时，先执行 ``enter``\ ；入口转换\ （``[*] -> ChildState``\ ）随后决定哪个子状态变为活动状态；普通 ``during before`` 在该决策之后、被选中子状态进入之前执行
   2. **期间**：叶子子状态活动时，切面动作（``>> during before/after``）围绕子状态的 ``during`` 动作执行
   3. **退出**：离开复合状态时，活动的子状态首先退出，然后执行普通 ``during after`` 和复合状态的 ``exit``
...
每个转换可以是普通转换、事件触发转换、守卫触发转换，也可以使用组合 trigger。组合 trigger 是由事件项和方括号守卫项按顺序组成的 ``+`` 链。它只是一层语法糖：模型构建阶段会把它展开为普通的自动伪状态和普通转换；运行时、验证工具、可视化和代码模板随后看到的都是展开后的模型。
...
**语法：** ``[*] -> target_state [trigger] [effect { operations }] ';'``
...
.. note::
   当从外部进入复合状态时，状态 ``enter`` 动作先执行，然后入口转换决定哪个子状态变为活动状态。守卫条件（如果存在）会在该复合状态的普通 ``during before`` 动作之前评估。如果守卫为真，则先执行转换效果（如果存在），再执行普通 ``during before``，最后进入被选中子状态。
...
**语法：** ``from_state -> to_state [trigger] [effect { operations }] ';'``
...
.. code-block:: fcstm
   ...
   Active -> Inactive : [timeout > 100];           // 守卫别名
   Slow -> Fast :: speed_up + [ready > 0] + ack;   // 组合 trigger
...
**语法：** ``from_state -> [*] [trigger] [effect { operations }] ';'``
...
.. code-block:: fcstm
   ...
   Complete -> [*] :: finish_event + ack;          // 退出组合 trigger
...
组合 trigger 是由两个或更多 trigger 项组成的有序序列。事件项匹配当前周期中存在的事件；重复的相同事件项是允许的，并保持 presence-based 语义，也就是一个匹配输入事件可以满足这段重复项链，尽管 inspect 会把它报告为疑似笔误。守卫项写成 ``[condition]``\ ，并且必须在伪状态链中的对应位置求值为真。原始转换上的效果（如果有）只挂到最后一条生成转换上，因此组合链失败时会像手写伪状态链一样回滚。
...
上面示例中的 ``h...``\ 只是确定性哈希后缀的占位写法。实际生成名只包含合法 identifier 字符；不要在手写伪状态名里写入字面量 ``...``\ 。

精确的生成状态名是确定性的，并以 ``__combo_`` 开头。用户手写状态名不能使用这个保留前缀。生成状态也会带有人类可读的 ``named`` 显示标签，所以可视化输出会如实显示这个自动伪状态，而不是把它隐藏掉。
...
.. code-block:: fcstm
   ...
   S -> T :: E1 + E2;                     // 源状态本地事件序列
   S -> T :: E1 + [x > 0] + E2;           // 事件、守卫、事件
   S -> T : E1 + /Bus.E2 + [x > 0];       // 链事件加绝对事件
   S -> T : [x > 0] + [y > 0];            // 全守卫链使用 ':'
   S -> T : [x > 0];                      // 纯守卫别名，等价于 ': if [...]'
   [*] -> S : Boot + Ack;                 // 入口组合
   S -> [*] :: Done + Ack;                // 退出组合

**无效形式：**

有些无效形式是语法错误；而 ``/Root.Bus.E1``\ 这类作用域路径错误可以通过语法解析，但会在模型校验阶段被拒绝，因为绝对路径已经从根状态下方开始，不再重复根状态名。

.. code-block:: fcstm

   S -> T :: [x > 0] + [y > 0];           // 全守卫组合不能用 '::'
   S -> T :: E if [x > 0];                // 应写成 ':: E + [x > 0]'
   S -> T :: E1 + : E2;                  // 不要在链中混入新的前缀
   !* -> T :: E1 + E2;                   // 强制转换不能使用组合 trigger
...
.. note::
   事件路径从根状态解析，允许对事件位置进行显式控制。不要在 ``/`` 之后重复根状态自身的名字；在 ``state Root`` 中应写 ``/Bus.E1``\ ，而不是 ``/Root.Bus.E1``\ 。
...
6. **组合命名空间**\ ：用户手写状态不能以 ``__combo_`` 开头，因为这个前缀保留给生成的组合伪状态
...
.. important::
   ...
   **场景 1：初始进入**\ （``HierarchyDemo -> Parent -> ChildA``）
   ...
   2. ``Parent.enter``\ （如果定义）
   3. ``Parent`` 选择其初始转换（``[*] -> ChildA``\ ）
   4. ``Parent.during before`` 执行（``execution_log += 100``）
   5. ``ChildA.enter``\ （如果定义）

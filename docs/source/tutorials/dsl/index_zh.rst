...
普通 ``.fcstm`` 文件先写 persistent variable，再写唯一 root ``state``。实际模型通常把 root 写成 composite，因为它需要拥有 child states 和 initial transition。
...
``def`` 声明的是模型持久状态，不是某个 block 内的临时变量。root state 随后成为 child state、event、transition、lifecycle action 和 import 的 ownership tree。
...
``: if`` 后面是 condition expression。``temperature`` 是 numeric expression；比较表达式把 numeric value 桥接成 condition。
...
.. list-table:: 下一步
   ...
   * - 建模多 term trigger
     ...
     - Combo transition 会展开成 pseudo relay states，同时保留作者语义。
   * - 用一个声明覆盖多个 source

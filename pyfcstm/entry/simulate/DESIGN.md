...
- [ ] 扩展 `/cycle` 命令支持重复次数参数：`/cycle [count] [events...]`
- [ ] `count` 为可选整数参数，默认值为 1
- [ ] 示例：`/cycle 5` - 执行 5 次周期（无事件）
- [ ] 示例：`/cycle 3 Start` - 执行 3 次周期，每次触发 Start 事件
- [ ] 示例：`/cycle 10 Start Stop` - 执行 10 次周期，每次触发 Start 和 Stop 事件
- [ ] 在每次周期后显示当前状态（可选：仅显示最后一次）
- [ ] 添加参数验证（count 必须为正整数）
- [ ] 更新 `/help` 命令文档
- [ ] 添加单元测试覆盖新功能

...
本页是面向事实查询的 FCSTM DSL reference。它对照当前拆分后的 grammar 文件，
...
同步说明：本页与英文 reference 按 section、表格行和语法事实逐项对应。中文段落会保留
grammar rule、token、CLI option 和代码 literal 的英文原文，但普通说明会给中文解释；
review 时应以 anchor、表格行、示例和语法事实是否对应为准，而不是用中英文自然换行数判断是否漏译。
复核时建议优先比较：

* section anchor 是否一一对应；
* syntax / diagnostics 表格行是否一一对应；
* checked example 与 expected diagnostics 是否一致；
* C/C++ target-risk wording 是否保持同一边界。
...
.. list-table:: 语法族
   ...
   * - 导入
     - ``import "path" as Alias`` 和 mapping block
     - :ref:`dsl-import-forms-zh`
   * - 诊断措辞
     - target-specific warning，尤其是 C/C++ deployment-profile risk
...
.. list-table:: 词法形式
   ...
   * - 标识符（identifier）
     - ``[a-zA-Z_][a-zA-Z0-9_]*``
     - 用于 variable、state、event、action name、alias 和 path part。
   * - 字符串（string）
     - single 或 double quoted strings
     - Import path 和 ``named`` label 使用 string；常见 escape sequence 由 lexer 处理。
   * - 注释（comment）
     - ``/* ... */``、``// ...``、``# ...``
     - Multiline comment 在特定 lifecycle form 中可以成为 abstract-action documentation。
   * - 关键字（keyword）
     - ``def``、``state``、``pseudo``、``event``、``import``、``enter``、``during``、``exit``、``abstract``、``ref``
     - Keyword 由 lexer rule 保留，不能作为普通 identifier 使用。
   * - 紧凑 import token
     - selector patterns and target templates
     - Compact form 在 import-specific lexer mode 中 tokenized，对 whitespace 敏感；``$0`` /
...
* Initializers 使用 ``init_expression``。这个子集接受 literal、math constant、arithmetic、
  bitwise operator 和 unary math function，但不接受 runtime variable reference，也不接受
  C-style ternary expression。
...
这些 forms 不是普通 top-level ``def`` declarations。它们让 imported modules 在被 host
重写之前暴露 assembly-time constants 或 initial values。
...
``chain_id`` 是可选 ``/`` 后接一个或多个 dotted identifiers。Source-private signals 使用
local events，containing protocols 使用 chain paths，全局拥有的事件使用 root paths。
...
Operator precedence 按 grammar rule order 从紧到松：

* parentheses / literals / functions；
* unary signs 和 power；
* multiplicative、additive、shift；
* bitwise ``&`` / ``^`` / ``|``；
* comparisons、condition equality / ``iff``；
* ``and``、``xor``、``or``；
* implication 和 ternary forms。
...
``ref`` 指向 named lifecycle action path，不指向 state 或 event。Doc-comment abstract forms
使用 multiline comment 作为 documentation metadata；上表中的 optional ``Name`` 是说明文字，
不是字面 ``Name?`` token。
...
Aspect actions 使用 ``>> during before`` 或 ``>> during after``。它们支持和 lifecycle
``during before/after`` 相同的 concrete、named、abstract、doc-comment abstract 与 ref families。
...
.. list-table:: Import syntax facts
   ...
   * - Target template
     ...
     - ``$0`` / ``${0}`` 表示完整 imported name；``$1`` / ``${1}`` 表示第一个 wildcard
       capture。Bare ``*`` 保留 imported name。
   * - Event mapping
...
File resolution、recursive loading、conflict detection、mapping precedence 和 model assembly
在 parsing 后的 Python import/model code 中实现。
...
Diagnostics 来自 syntax parsing、model validation、inspect analyzers 和 optional verification
phases。用户可见 DSL docs 必须保留每个 diagnostic 的 target scope。
...
.. list-table:: Diagnostics wording facts
   ...
   * - Python generated runtime
     ...
     - 除非 Python-specific diagnostic 明确说明，否则不要声称 Python generated code 有同样
       fixed-width 或 undefined-behavior risk。
...
下面的 matrix 面向 maintainer 和 reviewer。它有意放在用户语法事实之后，避免普通读者在查 DSL form 前先被 migration audit table 阻挡。

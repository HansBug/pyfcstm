...
.. code-block:: fcstm
   ...
   [*] -> Running :: startup_event;                // 带旧式链事件的入口
...
组合 trigger 是由两个或更多 trigger 项组成的有序序列。事件项会消耗当前周期中的事件；守卫项写成 ``[condition]``\ ，并且必须在伪状态链中的对应位置求值为真。原始转换上的效果（如果有）只挂到最后一条生成转换上，因此组合链失败时会像手写伪状态链一样回滚。

...
PR-1 已落库轻量框架和 R0 映射工具。PR-3 在此基础上新增 Python + Z3 基线 snapshot；整个调研系列统一禁止修改 `test/` 路径：

| 文件 | 作用 |
|---|---|
| `plan.md` | 子 PR 执行计划、验收命令和边界。 |
| `mapping.md` | R0 render mapping 输出结构和人工阅读指南。 |
| `probes.md` | probe runner 设计约束；当前包含 `env` 与 `python-z3-baseline` 入口说明。 |
...
- PR-3 不运行 C/C++、Java、Rust、Go、Node 或 TypeScript probe。
- PR-3 不修改 solver、verify、fixed 或模板语义。
- PR-3 不提交 exhaustive 原始输出。
- PR-3 的 Python 输出是 P3 / 无限精度 / 仿真兼容基线，不是默认定长 profile。
...
`numeric_render_mapping.py --check` 是 R0 mapping 的调研自检入口；`numeric_render_probe.py python-z3-baseline --check` 是 PR-3 baseline 的轻量契约自检入口。后续子 PR 也应继续使用 `tools/` / `research/` 下的自检和 probe 入口，而不是修改 `test/`。`results/local/` 被 `.gitignore` 忽略，适合放本机探测结果和后续 heavy probe 输出。

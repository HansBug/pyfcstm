...
核成员是已经提交的约束，其中可能早已包含变量代换、条件值和多段动作效果。
找到它的源码行，不等于恢复了这些构建步骤。因此，可选的构建记录直接在现有操作执行器
和表达式翻译器中产生，此时每个源码出现位置仍有真实的输入环境。
它不解析打印后的 Z3 公式来猜原程序，也不另写一套语句执行器。

编辑器中 Trim 动作的构建顺序可以直接观察：
...
.. list-table:: 有序读写关系
   ...
   * - ``refund = quantum - margin;``
     - 读取输入余量和共享参数，产生新的退款量版本。
   * - ``proposal = proposal - refund;``
     - 读取输入候选值和前一句刚写入的退款量。
   * - ``margin = margin + refund;``
     - 读取输入余量和同一个新退款量，不读取动作执行前的退款量。

代换得到 ``margin + (quantum - margin)``，Z3 可以将它化简为 ``quantum``。
显示化简结果与逐条检查算术证明规则是不同的能力。完整可运行例子和实际输出见
:doc:`../../how_to/bmc/index_zh`。

这里存在三种不同的身份。源码出现位置通过原始树中的路径标识一条语句或一个表达式子节点；
执行实例标识查询、帧、宏步分支和动作调用；值版本标识该次调用内的一次具体写入。
``x = x`` 即使不改变值表达式，仍产生新写入。``x + x`` 中两个子节点即使共用同一个
Z3 值，也仍是两个源码位置。重复引用同一个具名动作会共用源码，但各次输入值和局部变量
生命周期不同。参数在各帧共用一个符号，输入变量则逐步采样。

条件也是证据的一部分。操作分支保留有顺序的选择条件，包括对先前分支的排除。
分支汇合记录各选择条件提供的值版本，以及没有分支执行时保留的输入版本。
短路子表达式保留求值条件和运行时有定义性要求。守卫记录使用其实际
``after_action_block_index`` 对应的环境，不能把全部守卫都套到动作结束后的环境上。
热启动从自己的第零帧取值开始，不虚构更早的进入动作。编译器生成其他源状态的候选分支，
不表示那些分支会在选定轨迹中执行。

绑定检查器将局部翻译和版本关系与实际编译结果对齐。这能提供一致性证据，但不是独立的
编译器正确性证书，也不证明所有已裁剪分支确实不可达。尤其是检查器会复用产生记录时的
表达式翻译器，不能把捕获的元数据直接提升为无条件逻辑前提。
完整的无解解释仍需从选定核和固定背景出发，在有效作用域内推导，覆盖所有相关分支，
最终闭合到矛盾。

源码切片显式保留从保留位置到原始位置的映射，不能仅凭相等的结果值识别语句。
若模型加载器提供了语句范围，就可以展示该位置。即使没有单独提供子表达式的字符范围，
其子节点路径仍然有效。缺失的位置明确记为缺失，不编造坐标。

详细记录默认关闭，请求证据时不会暗中重新编译。绑定检查和等价核细化都需要显式调用，
各自使用共享总预算。完成某项检查，只能将这一项标为完成；未知可达性或预算耗尽，
都不能用来排除分支或宣称解释完整。

跨动作、跨帧阅读构建记录
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

帧记录宏步边界，不记录每一次赋值。编辑器中，第 2 步的 Trim 动作依次写入退款量、
候选值和余量。该分支真实的后状态约束将最后一次余量写入连接到 ``margin@3``，
第 3 步 Trim -> Ready 的守卫读取这个 ``margin@3``。如果第 2 步还有后续动作
覆盖余量，帧边界就必须连接到后一次写入。文本生成沿记录中的写入身份和最终版本连接，
不按表达式打印结果是否相等来猜，因此也不会丢失恒等赋值。

同一个具名动作调用两次时，源码相同，但执行身份和局部变量生命周期独立。
展示定义在同一候选分支内按变量编号，例如 ``temporary#1`` 和 ``temporary#2``。在两个调用之后求值的守卫读取第二次调用的输出。
嵌套分支显示在所属条件语句下面，汇合逐项列出选择条件及提供结果的写入，
没有分支执行时保留原值。未知可达性只是一项观察结果，不能用作排除分支的依据。
语句编号指向原始源码位置，增加无关入口变量不会改变这些编号。

控制条件也属于这份记录。Review -> Trim 要求当前状态为 Review、余量守卫成立，
并排除先前 Undo 迁移被接受。精确的叶位置显示为 ``active("path")@f``，
非叶入口显示为 ``control("path")@f``。cold 和 terminated 显式区分，
已进入的根叶状态保留必要的 ``!cold`` 条件。事件使用 ``event("path")@k``。参数跨步共享，外部输入逐步变化。
复合状态谓词可能包含其后代，帧边界的精确控制状态不能与宏步中曾进入的所有状态混淆。

一个容易讲错的反例是带额外守卫的高优先级事件迁移：事件可以存在，但该迁移的
接受条件仍不成立。因此，报告展示完整接受条件，不能直接说事件缺席。
这些依赖已经嵌入当前分支的有效条件，不是从选定核之外补进来的新前提。

最后，两个打印出来的候选分支不等于一条可行执行，也不等于穷尽性证明；它们可能
互斥或不可达。构建文本保留各分支作用域和有条件的帧边界等式，可达性、分支覆盖和
最终矛盾仍需另行检查。默认文本保留作者的表达式及准确读值绑定，
``expanded=True`` 增加实际展开公式，不替换原有绑定关系。
展开后的帧末不含局部 ``#n`` 或展示别名，列出全部保留的持久变量及保持项；
切片删除项明确披露。

排版与逻辑推导是不同操作。合取中中性的 ``true`` 可以去掉，但源码 ``x = x``
仍是一次定义，未修改的帧变量仍有保持等式。具名守卫和接受条件保留作用域与顺序。
精确的数值常量比较可以变为真或假，来源仍可查；不使用整体化简器改写动作的算术结构。
共享条件名只是公式缩写，不引入前提。展开这些名称不证明分支可达，也不完成无解论证。

...
``action.text_lines(names=None, expanded=False)``
    返回局部动作文本，保留原始源码语句，引用入口值、具体赋值及汇合结果。
    传入 ``core.symbols.names`` 可使用帧、输入和参数名称。
    动作和语句位置从 1 开始，嵌套位置依次表示语句、分支、语句；
    ``2.1.3`` 表示第 2 条语句的第 1 个分支中的第 3 条语句。
    切片后保留原始位置。内部 ``ConstructionValue.identifier`` 仍是从 0 开始、
    所有变量共用的图节点索引，不再当作变量版本直接打印。

``report.text_lines(expanded=False)``
    按帧和候选分支顺序返回选定来源组与构建记录，包括有效条件、状态名称、
    所属步骤的事件名称、优先级依赖、真实动作前缀位置上的守卫、跨动作引用及
    有条件的帧边界等式。帧边界状态与宏步内部经过的瞬态状态不同。
    父状态谓词可能覆盖多个控制状态编码；图例说明编码含义，不替换普通数值常量。
    终止步的边界事件约束也会展示。列出候选分支既不建立执行轨迹，也不证明分支覆盖。
    被排除分支的接受条件用于说明已嵌入的依赖，不增加新的选定前提。

两个文本接口均返回 ``Tuple[str, ...]``，不截断、不重新编译、不求解、
不修改共享符号登记表，也不写文件。``expanded`` 只能通过关键字传入，
默认 ``False``；传入非布尔值抛出 ``TypeError``。
``expanded=True`` 增加实际赋值展开式和明确标注的 Z3 化简，报告还会包含提交公式。
运算符保留原生记法。文本生成不认证记录正确性，需单独调用 ``report.check(...)``
并保留其已验证、无效或未知的状态。本次不增加命令行开关或公共 JSON 字段。

用法举例：``report.text_lines()`` 用共享帧符号连接相邻步骤；
``report.text_lines(expanded=True)`` 展示完整公式；
``action.text_lines(core.symbols.names)`` 定义一个独立动作的入口值。
``report.text_lines(expanded=1)`` 非法。两个不同候选分支中的
``x [action 1, after statement 1]`` 也不是同一个值：引用受所属查询、步骤和
候选分支限制。动作入口读值可能来自前一个动作，不能一律当作输入帧中的值。

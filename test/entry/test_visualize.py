...
class TestVisualizeHelpers:
    def test_resolve_visualize_output_path_append_suffix(self, input_code_file):
        output_path = visualize_module.resolve_visualize_output_path(
            input_code_file, "demo/output", "svg"
        )
        assert output_path.name == "output.svg"
        assert output_path.is_absolute()

    def test_resolve_visualize_output_path_reject_mismatch_suffix(
        self, input_code_file
    ):
        with pytest.raises(ClickErrorException, match="does not match render type"):
            visualize_module.resolve_visualize_output_path(
                input_code_file, "demo/output.png", "svg"
            )
    ...
    def test_resolve_renderer_backend_auto_prefers_remote_when_local_unavailable(
        self, monkeypatch
    ):
        class DummyRemote:
            def check(self):
        ...
        monkeypatch.setattr(
            visualize_module, "create_local_plantuml_backend", _local_fail
        )
        monkeypatch.setattr(
            visualize_module,
            "create_remote_plantuml_backend",
            lambda **kwargs: DummyRemote(),
        )
...
class TestEntryVisualize:
    def test_visualize_render_and_open_success(self, input_code_file, monkeypatch):
        outputs = []
        monkeypatch.setattr(
            visualize_module, "render_plantuml_diagram", _mock_render(outputs)
        )
        monkeypatch.setattr(
            visualize_module, "open_diagram_with_default_app", lambda _: (True, "")
        )
        ...
        assert (
            "Opened rendered diagram with the system default viewer." in result.stdout
        )
        assert outputs
    ...
    def test_visualize_no_open_with_explicit_output(self, input_code_file, monkeypatch):
        ...
        monkeypatch.setattr(
            visualize_module, "open_diagram_with_default_app", _should_not_open
        )
    ...
    def test_visualize_open_skipped_in_headless_mode(
        self, input_code_file, monkeypatch
    ):
        outputs = []
        monkeypatch.setattr(
            visualize_module, "render_plantuml_diagram", _mock_render(outputs)
    ...
    def test_visualize_strict_open_failure(self, input_code_file, monkeypatch):
        ...
        assert result.exitcode != 0
        assert "Failed to open rendered diagram automatically." in (
    ...
    def test_visualize_missing_plantumlcli(self, input_code_file, monkeypatch):
        def _raise():
            raise ClickErrorException(
                'Python package "plantumlcli" is not installed or failed to import: missing'
            )
        ...
        assert result.exitcode != 0
        assert 'Python package "plantumlcli" is not installed or failed to import' in (
            result.stderr or result.stdout
        )
    ...
    def test_resolve_renderer_backend_local_failure(self, monkeypatch):
        ...
        with pytest.raises(
            ClickErrorException, match="Local PlantUML renderer is unavailable"
        ):
    ...
    def test_resolve_renderer_backend_remote_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(
            visualize_module, "create_remote_plantuml_backend", _remote_fail
        )
        with pytest.raises(
            ClickErrorException, match="Remote PlantUML renderer is unavailable"
        ):
    ...
    def test_resolve_renderer_backend_remote_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            visualize_module, "create_remote_plantuml_backend", lambda **kw: _OkRemote()
        )
    ...
    def test_resolve_renderer_backend_auto_both_fail(self, monkeypatch):
        ...
        with pytest.raises(
            ClickErrorException, match="No usable PlantUML renderer found"
        ):
    ...
    def test_resolve_renderer_backend_unexpected_type_propagates(self, monkeypatch):
        ...
        monkeypatch.setattr(
            visualize_module, "create_local_plantuml_backend", _local_typebug
        )
        with pytest.raises(TypeError, match="wrong arg type"):
            visualize_module.resolve_renderer_backend("local")

    def test_render_plantuml_diagram_success_returns_renderer(
        ...
        name = visualize_module.render_plantuml_diagram(
    ...
    def test_load_plantumlcli_classes_import_error_propagates(self, monkeypatch):
        ...
        with pytest.raises(
            ClickErrorException, match="not installed or failed to import"
        ):
    ...
    def test_render_plantuml_diagram_failure(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(
            ClickErrorException, match="Failed to render diagram with plantumlcli"
        ):
    ...
    def test_render_local_plantuml_on_windows_uses_materialized_source(
        self, tmp_path, monkeypatch
    ):
        class DummyBackend:
            java = "java.exe"
        ...
        def _run(command, **kwargs):
            ...
            (output_dir / "diagram.png").write_bytes(b"png-data")
            return type(
        ...
        assert output_file.read_bytes() == b"png-data"
        assert commands[0][0][:4] == ["java.exe", "-jar", "plantuml.jar", "-tpng"]
        assert commands[0][1]["check"] is False

    def test_render_local_plantuml_on_windows_reports_process_failure(
        self, tmp_path, monkeypatch
    ):
        class DummyBackend:
            java = "java.exe"
        ...
        with pytest.raises(OSError, match="status 1: invalid diagram"):
            visualize_module._render_local_plantuml_on_windows(
                DummyBackend(),
                tmp_path / "out.png",
                "png",
    ...
    def test_render_local_plantuml_on_windows_requires_generated_file(
        self, tmp_path, monkeypatch
    ):
        class DummyBackend:
            java = "java.exe"
        ...
        with pytest.raises(FileNotFoundError, match="produced 0 'png' files"):
            visualize_module._render_local_plantuml_on_windows(
                DummyBackend(),
                tmp_path / "out.png",
                "png",
    ...
    def test_render_local_plantuml_on_windows_converts_pdf_via_svg(
        self, tmp_path, monkeypatch
    ):
        class DummyBackend:
            java = "java.exe"
        ...
        def _run(command, **kwargs):
            ...
            output_dir = pathlib.Path(command[command.index("-o") + 1])
            (output_dir / "diagram.svg").write_text("<svg/>", encoding="utf-8")
            return type(
                "Completed", (), {"returncode": 0, "stdout": "", "stderr": ""}
            )()

        def _svg2pdf(*, bytestring, write_to):
            assert bytestring == b"<svg/>"
            pathlib.Path(write_to).write_bytes(b"%PDF-1.4\n")

        monkeypatch.setattr(visualize_module.subprocess, "run", _run)
        monkeypatch.setitem(
            sys.modules,
            "cairosvg",
            types.SimpleNamespace(svg2pdf=_svg2pdf),
        )
    ...
    def test_render_plantuml_diagram_routes_windows_local_backend(
        ...
        class DummyBackend:
            ...
            def dump(self, *args, **kwargs):
                raise AssertionError(
                    "Windows local rendering must bypass plantumlcli.dump"
                )

        calls = []

        def _render(backend, output_file, render_type, plantuml_output):
            calls.append((backend, output_file, render_type, plantuml_output))
            output_file.write_bytes(b"png-data")

        monkeypatch.setattr(visualize_module.sys, "platform", "win32")
        monkeypatch.setattr(
            visualize_module,
            "create_local_plantuml_backend",
            lambda **kw: DummyBackend(),
        ...
        monkeypatch.setattr(
            visualize_module, "_render_local_plantuml_on_windows", _render
        )
        output_file = tmp_path / "out.png"

        renderer = visualize_module.render_plantuml_diagram(
            "@startuml\n@enduml",
            output_file,
            "png",
            "local",
        ...
        assert renderer == "local"
        assert output_file.read_bytes() == b"png-data"
        assert calls[0][2:] == ("png", "@startuml\n@enduml")
    ...
    def test_render_plantuml_diagram_missing_output(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            visualize_module,
            "create_local_plantuml_backend",
            lambda **kw: SilentBackend(),
        )
        with pytest.raises(
            ClickErrorException, match="plantumlcli reported success but no output file"
        ):
            visualize_module.render_plantuml_diagram(
                "@startuml\n@enduml",
                tmp_path / "out.png",
    ...
    def test_open_diagram_in_headless_environment(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PYFCSTM_NO_GUI", "1")
        opened, reason = visualize_module.open_diagram_with_default_app(
            tmp_path / "noop.png"
        )

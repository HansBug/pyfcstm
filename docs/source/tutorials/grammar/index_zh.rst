...
**条件关键字：** ``if``、``and``、``or``、``not``、``implies``、``xor``、``iff``
...
``^`` 会作为数值位异或运算符高亮。它不是守卫条件中的布尔异或写法；
布尔异或请使用 ``xor`` 关键字。

**比较运算符：** ``<``、``>``、``<=``、``>=``、``==``、``!=``

**逻辑运算符：** ``!``、``&&``、``||``、``=>``、``and``、``or``、``not``、``implies``、``xor``、``iff``
...
.. note::
   在任何具体操作块中（``enter { ... }``、``during { ... }``、
   ``exit { ... }`` 或 ``effect { ... }``），如果给一个此前未声明的名字
   赋值，就会创建一个块级临时变量。它可以被同一块中后续语句复用，但在
   该块结束后就会消失，也不能在其他块或守卫中引用。

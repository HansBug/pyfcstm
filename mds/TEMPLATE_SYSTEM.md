...
### 阶段 1：打通模板源码目录与包内发布链路

目标：

- 建立根目录 `templates/`
- 增加 `templates/README.md` 与 `templates/README_zh.md`
- 增加 `pyfcstm/template/`
- 调整打包配置，把模板 zip、索引文件与释放模块带进安装包
- 在 `Makefile` 中增加 `templates_package`
- CLI 支持 `--template`

这一阶段不必先实现 Python 模板全部逻辑，但必须先打通“模板源码目录 -> zip -> `pyfcstm/template/` -> 解压目录 -> renderer”这条分发路径。

### 阶段 2：补 statement renderer

目标：

- 支持 Python 语句块渲染
- 把块级临时变量语义设计清楚
- 为模板层提供稳定的 statement generation 能力

这是最关键的基础设施阶段。

### 阶段 3：实现 `python_hardcoded` builtin template
...
- 将若干 `SimulationRuntime` 关键样例迁移为“生成后执行”的测试
- 验证语义一致性

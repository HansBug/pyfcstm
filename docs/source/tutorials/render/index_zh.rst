...
渲染器会把空的 ``config.yaml``\ 视为一个空 mapping。未知 top-level key 会提前报错，错误信息会包含未知 key、允许 key 列表和 config 路径。

``globals``\ 、``filters``\ 和 ``tests``\ 下的条目支持三种声明式形式：

- ``type: template``\ ：从内联 Jinja2 snippet 构造 callable。它需要 ``template``\ 字段，也可以使用有序 ``params``\ 。
- ``type: import``\ ：根据 ``from``\ 字段 import Python 对象。这是 trusted-template code boundary，不应用于不可信模板。
- ``type: value``\ ：暴露 ``value``\ 字段里的 literal value。

目标语言专属 helper 应放在对应模板自己的 ``config.yaml``\ 中。例如，C runtime helper 由 C 和 C Poll 模板注册，而不是由默认 renderer environment 注入。
...
- ``c`` 要求集成层先把本周期事件整理出来，再把 event-id 集合传给 ``cycle(...)``
- ``c_poll`` 要求集成层先安装生成出来的 ``check_xxx`` hooks，然后运行时在
  ``cycle()`` 内部做迁移判定时按需调用这些 hook

这意味着两者对“谁来观察事件”有不同分工：

- 在 ``c`` 里，应用层负责事件采集与归一化，然后把结果喂给运行时
- 在 ``c_poll`` 里，运行时自己掌握事件观察时机，并通过 event-check 表去问
  “当前这一拍这个事件是否成立”

这也正是 ``c_poll`` 更贴近很多真实控制系统模式的原因。它通常更适合：

- 按固定 control tick / scan cycle 执行的循环系统
- 类 PLC 的扫描式逻辑，即每拍先读取当前输入，再基于当前输入判定迁移
- 事件是否成立来自输入引脚、传感器快照、现场总线镜像、共享 process image
  之类“当前世界状态”的集成方式
- 外围系统本身并没有现成事件分发层，更自然的接入方式是“按需检查当前条件”

实际选型时，可以这么理解：

- 如果你的外围系统已经有清晰的事件收集 / 分发步骤，并且更适合把一组明确 event ids
  喂给状态机，那么选 ``c``
...
模板工作至少应该分层测试，而不是只看“能不能渲染出文件”。组合 transition trigger 不需要模板专用 primitive：模板拿到 ``StateMachine`` 模型时，组合 trigger 已经展开为普通伪状态和普通转换。因此模板测试应该验证展开后的伪状态链模型，而不是在 renderer 或模板里增加特判。

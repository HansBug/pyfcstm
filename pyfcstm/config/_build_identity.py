...
_TRANSPORT_PREFIX = "pyfcstm-build-info-v1:"
...
def load_build_identity_file(path: os.PathLike) -> BuildIdentity:
    ...
    transport_identity = None  # type: Optional[BuildIdentity]
    body = tree.body
    docstring = ast.get_docstring(tree, clean=False)
    if docstring is not None:
        transport_identity = _load_build_identity_transport(docstring)
        body = body[1:]
    ...
    identity = _validate_identity_values(values)
    if transport_identity is not None and transport_identity != identity:
        raise BuildInfoDataError(
            "build info transport payload does not match generated assignments"
        )
    return identity
...
def _build_identity_transport(identity: BuildIdentity) -> str:
    """Encode validated identity values for frozen-module static loading."""
    _validate_identity_values(identity.values())
    values = tuple(identity.values()[field] for field in BUILD_INFO_FIELDS)
    encoded = ascii(values).encode("ascii").hex()
    return _TRANSPORT_PREFIX + encoded


def _load_build_identity_transport(payload: str) -> BuildIdentity:
    """Decode static identity data without executing its generated module."""
    if type(payload) is not str or not payload.startswith(_TRANSPORT_PREFIX):
        raise BuildInfoDataError("build info transport payload is missing or invalid")
    encoded = payload[len(_TRANSPORT_PREFIX) :]
    try:
        serialized = bytes.fromhex(encoded).decode("ascii")
    except (ValueError, UnicodeError) as err:
        # ValueError: malformed hex; UnicodeError: decoded bytes are not ASCII.
        raise BuildInfoDataError("build info transport encoding is invalid") from err
    try:
        values = ast.literal_eval(serialized)
    except (SyntaxError, ValueError) as err:
        # SyntaxError and ValueError describe malformed Python literal data.
        raise BuildInfoDataError("build info transport literal is invalid") from err
    if type(values) is not tuple or len(values) != len(BUILD_INFO_FIELDS):
        raise BuildInfoDataError("build info transport schema is invalid")
    return _validate_identity_values(dict(zip(BUILD_INFO_FIELDS, values)))
...
def write_build_identity_file(path: os.PathLike, identity: BuildIdentity) -> None:
    ...
    lines = [
        '"""{}"""'.format(_build_identity_transport(identity)),
        "# Generated by tools.write_build_info. Do not edit.",
        "",
    ]

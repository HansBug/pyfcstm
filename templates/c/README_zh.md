...
- [ ] 从生成出来的公开 C API 中移除字符串事件提交方式。
- [ ] 为 transition 直接生成 `event_id` 常量，而不是在运行时持有
      `event_path` 再做查找。
- [ ] 将 `cycle()` 直接改为接收整数事件 id。
- [ ] 在 `machine.h` 中生成公开的事件 id 宏，调用方无需再做字符串解析。
- [ ] 从 hot start API 中移除字符串状态查找，改为直接接收 state id。
- [ ] 在 `machine.h` 中生成公开的状态 id 宏，方便 hot start 等性能敏感路径
      直接使用。
- [ ] 将当前每次状态推进都会付费的小型 helper 尽量内联或直接展开生成。

### Phase 2：围绕 id 输入改造事件集合表示

- [ ] 让基于整数 id 的 `cycle()` 成为头文件里唯一暴露的事件输入方式。
- [ ] 替换当前线性的 event-set 判定逻辑，并且不能因为采用位运算就引入一个
      很小的固定事件上限。
- [ ] 当状态机总事件数较小或中等时，优先使用紧凑 bitset 作为快速路径。
- [ ] 当事件空间较大时，退化为去重后的整数 id 数组，或开放寻址哈希集合等更
      合适的结构，而不是强行只用 bitset。
- [ ] 尽量把事件集合需要的临时存储做成 machine 内可复用 scratch 区，避免
      每个 cycle 都做堆分配。
...
- [ ] 生成出来的 `machine.h` 保持小而面向使用者。
- [ ] 生成出来的 `machine.c` 可以明确偏向运行性能，而不必优先照顾人工可读性。
- [ ] 生成出来的公开 API 直接使用整数 id 提交事件。
- [ ] `machine.h` 暴露事件 id 和状态 id 宏，使用方不需要依赖运行时字符串查找。
- [ ] 事件处理不能因为只使用位运算而引入一个很小的固定上限。
- [ ] 现有运行语义、回滚行为和 hook 语义保持正确。

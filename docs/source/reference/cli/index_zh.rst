...
JSON 写入标准输出；HTML 写到请求路径。使用 ``--open`` 且省略路径时，写入一个由
文档内容派生的临时路径——浏览器窗口比命令本身活得久，因此该文件不会被删除；再次
展示同一幅图形会复用同一个文件，而不是新增一份。

命令无法产出的输出后缀属于用法错误：请使用 ``.json`` 或 ``.html``\ ，或用
``--format`` 显式指定。``--open`` 找不到浏览器时，仍会先打印查看器路径再报错，
以便手动打开。
...
* ``--open`` 需要 Chromium 系浏览器。没有浏览器时，命令会先写出 HTML，
  再报告类型化能力错误；只生成文件时不要使用 ``--open``。
...
上表中的命令、文件名、选项、错误关键字保持原文，是为了可复制和可搜索。普通说明应使用中文术语；如果后续继续扩写本页，应避免把命令、选项、输出、失败这类概念反复写成英文普通说明。

...
`cpp` 是早期一等内置 C++ 模板。它会生成与 `templates/c/` 相同的 C99 执行 core，
并额外生成 `machine.cpp` / `machine.hpp` wrapper 文件，提供覆盖公开 C API 的
...
`template.json` 在 C++ wrapper surface 仍处于早期推广期时会有意保留
...
原生工具链检查应让 `cpp` 和 `c`、`c_poll`、`cpp_poll` 一起进入仓库级矩阵。C++
harness 可以链接生成出的 C core，但被测试的公开集成面仍然是 C++ wrapper。Generated
README 中的构建示例需要和矩阵实际覆盖的 profile 保持一致，包括 CMake、GCC/G++、
Clang/Clang++、no-exception、no-RTTI 以及适用时的 no-heap 形式。

部署安全表述沿用 `templates/c/` 的 C-family 边界。该模板提供的是非认证工程基线，不是认证包：
C99 core、C++98 wrapper、调用方拥有对象与无堆集成、共享语义对齐和原生工具链矩阵证据。不要把 generated output 描述为 MISRA、AUTOSAR、DO-178C、IEC 61508、ISO 26262 或其他认证 ready。Numeric inspect warning 是默认 C/C++ deployment-profile diagnostics，不是 target-independent FCSTM model error，也不是 Python 模板风险。后续 BitVec、BMC、fixed-point、numeric-profile、checked-arithmetic 或 generated failure channel 工作属于 verify 和 codegen 设计路线。

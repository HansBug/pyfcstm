...
def get_cn_cases():
    return [
        file for file in glob.glob(get_testfile('cncode_*.c'))
...
def get_zh_cases():
    return [
        file for file in glob.glob(get_testfile('zhcode_*.c'))
...
class TestUtilsDecode:
    @pytest.mark.parametrize(['code_file'], [
        (file,) for file in get_cn_cases()
    ])
    def test_auto_decode_for_simplified_chinese(self, code_file, cn_expected_text, text_aligner):
    ...
    @pytest.mark.parametrize(['code_file'], [
        (file,) for file in get_zh_cases()
    ])
    def test_auto_decode_for_traditional_chinese(self, code_file, zh_expected_text, text_aligner):
        actual_text = auto_decode(pathlib.Path(code_file).read_bytes())
        text_aligner.assert_equal(
